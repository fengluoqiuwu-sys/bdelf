...
- **禁止**：训练、自动训练闭环、开 subagent、直接 `sbatch`、任何 `sar project`

### Cursor（`.cursor/` 配置）

- **自动训练闭环**：auto-train（Claude 禁用）
- **研究探索**：research-scout / paper-ingest / idea-kickoff（subagent 创建）
- **计算管理**：vram-probe / compute-ops / `slurm-auto-run`（`sar`）
...
| `compute-local.md` | 本机 RTX 5080 只推理；不抢锁 |
| `compute-remote-common.md` | 远端 common；单次卡数上限，无合计限额 |
| `compute-remote-slurm.md` | 远端 Slurm 必须经 `sar`；禁止直接 sbatch |
...
| `temp-layout.md` | temp/ 布局（ideas / sar-jobs / 本机与 common agent） |
...
1. 遵守所有 rules/ 中的约束
2. 使用 skills/ 中定义的工作流
3. 将训练 / 未授权远端 / 研究探索请求引导到 Cursor

## 更新

从 Cursor 配置同步时：对齐计算与 checkpoint 约定，并保持 Claude 范围（不训练、不开 subagent、远端须授权）。不要原样复制 Cursor 的 auto-train / train-ops / slurm-auto-run skill。
