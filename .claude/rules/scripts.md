...
- 用户授权的远端：`ssh ovan-server '~/bin/sar …'`、`bash scripts/launch-train.sh <name> --server <服务名> --gpus …`、`bash scripts/launch-eval.sh <name> --server <服务名> --gpus … -- --run …`、`bash scripts/sync.sh <服务名> push|pull|…`、`ssh <SSH主机> 'cd ~/source/bdelf && …'`（服务名见 `scripts/servers.csv`）。
- 禁止 `cd scripts && python foo.py`。
- `scripts/` 下 Python 用 `repo_env.ensure_repo_root()`；`slurm/*.py` 自行 `chdir` 到仓库根。
- `<服务名>` 与工作目录见 `scripts/servers.csv`。「名字」可写 `别名:SSH主机`；脚本 / `--server` / `logs/<服务名>/` 用服务名，`ssh` 用 SSH 主机。
...
- common 远端：`ssh <SSH主机>`；占 GPU 须 `scripts/launch-train.sh` / `scripts/launch-eval.sh`（`--server` + `--gpus`），写 `temp/agent` 与 `logs/<服务名>/<时间戳>/`；无 `remote_status.sh`（见 rule「远端 common 计算约束」）。
- 本机读远端日志：push（如需）→ `ssh <SSH主机> 'cd ~/source/bdelf && .venv/bin/python slurm/tail_remote_logs.py …'`（可加 `--server <服务名>`）。
