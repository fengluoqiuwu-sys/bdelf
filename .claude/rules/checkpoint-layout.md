...
| 选用 latent（HF） | `artifacts/latent/{model}/{tag}/` | `scripts/export_latent_artifact.py` 从训练 latest 导出（EMA 熔进权重）；**随后离线**写入逐维 μ 白化（默认 2^20 有效 token，见 `scripts/compute_latent_whiten.py`）；加载只读 |
...
`artifacts/latent/{model}/{tag}/` 供下游 / HuggingFace 固定引用。导出：`scripts/export_latent_artifact.py --run full/latent/<model>/<hash>`（只写最终参数与模型 config，**默认再离线估计逐维 μ 白化**）。已有 tag 补算：`scripts/compute_latent_whiten.py --latent-model <model> --tag <tag>`（默认 1048576 有效 token，不跑全集）。加载：`load_latent_artifact(latent_model, tag)`。训练保存若目标落在 `artifacts/latent/` 会拒绝（不影响 `artifacts/ace` 等）。白化不进 VAE 训练；BELF/RELF 的 `whiten: true` 读 artifact 里的 `whitening_mean`/`whitening_std`，缺则单位仿射。
