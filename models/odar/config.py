"""ODAR (Orthogonal Dual-Axis Repair; ELF backbone + DMA-H) configuration."""
...
class FL_ODARConfig(PretrainedConfig):
    """Configuration for ODAR config (ELF-aligned SC-CFG + DMA-H)."""
    ...
    _YAML_REQUIRED = frozenset(
        {
            ...
            "dma_mode",
            "dma_tau",
            # dma_commit_lambda 可选：默认 0；不进 required，旧 YAML 可加载
        }
    )
    ...
    def __init__(
        ...
        # DMA-H：日程化 embedding round-trip（进训练 hash）
        dma_t0: float = 0.5,
...
CONFIG_CLS = FL_ODARConfig


@dataclass
class ODARSamplingConfig:
    """ODAR inference configuration (supports SC-CFG guidance)."""
    ...
    @classmethod
    def from_dict(cls, cfg: dict) -> "ODARSamplingConfig":
