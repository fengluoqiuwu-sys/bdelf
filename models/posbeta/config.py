"""Posβ（posbeta）：ELF 骨架 + 位置相关插值 β_t^(ℓ)。"""
...
class FL_PosBetaConfig(PretrainedConfig):
    """Posβ：ELF 双分支 + 前缀更净、后缀更噪的位置相关插值。"""

    model_type = "fl_posbeta"
    ...
    def backbone_kwargs(self) -> Dict[str, Any]:
        return {
            ...
            "pos_beta_kappa": self.pos_beta_kappa,
        }


CONFIG_CLS = FL_PosBetaConfig


@dataclass
class PosBetaSamplingConfig:
    """Posβ 推理采样（SC-CFG / ACE；与训练 κ 无关）。"""
    ...
    @classmethod
    def from_dict(cls, cfg: dict) -> "PosBetaSamplingConfig":
