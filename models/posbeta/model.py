"""Posβ（posbeta）：ELF 骨架上的位置相关插值。

在冻结 encoder 连续空间实现 DFM §4.1 写出但未实验的
``z[ℓ] = β_t^(ℓ) x[ℓ] + (1-β_t^(ℓ)) ε[ℓ]``，``β_t^(ℓ) = t^{1+κ u(ℓ)}``，
前缀更干净、后缀更噪。κ=0 时与 ELF 各向同性路径逐公式一致。
推理仍共享全局 Euler 时钟；``x̂→v`` 按位置用 ``β̇/(1-β)``。
"""
...
from models.posbeta.ace import (
    ace_step_active,
    apply_ace_steer,
    parse_ace_step_range,
    resolve_ace_steering,
)
from models.posbeta.config import FL_PosBetaConfig
from models.posbeta.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.posbeta.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _PosBetaBackbone(nn.Module):
    """Posβ：ELF 双分支 + 位置相关插值（κ=0 退回各向同性）。"""
    ...
    def _pos_beta(
        ...
        （``_x_to_v`` / ``_denoise_path`` 已短路到 ELF 公式）。
        """
    ...
    def _sde_step(
        ...
        if abs(self.pos_beta_kappa) < 1e-12:
            ...
            z_back = alpha * z + (1.0 - alpha) * eps * (remain_pos / remain_iso)
        v, x_pred = self._forward_sample(
    ...
    def _decode_tokens(
        ...
        *,
        ...
        if not torch.isfinite(logits).all():
            raise RuntimeError("Posβ decode produced non-finite logits")
        if temperature <= 0:
    ...
    def generate(
        ...
        *,
        ...
        del bos_token_id, prefix_tokens  # 无条件 Posβ
        cfg = sampling_cfg or {}
        if seqlen is None:
        ...
        if not torch.isfinite(z).all():
            raise RuntimeError("Posβ sampling produced non-finite latents")
