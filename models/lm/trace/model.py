"""TrACE（Tracked ACE）：ELF 骨架 + 训练期抗吸引子正则。

架构与 ELF 相同（冻结 T5 encoder、DiT 去噪、mixed_branch、SC-CFG）。
差别：去噪行对 clean 预测的池化 ``u`` 加
``L_attr = ReLU(<u, d>)^2``（``d`` stop-grad）。短 FT 可冻 ``d``；
长训由 ``models.lm.trace.track`` 低频用 EMA 权重跑 ACE Alg.1 再 EMA 更新 ``d``。
推理默认 ACE=off，不再减 ``λd``。

References:
  - ELF: https://arxiv.org/abs/2605.10938
  - ACE / anti-attractor: https://arxiv.org/abs/2607.00588
"""
...
from models.lm.trace.ace import apply_ace_steer, resolve_ace_steering
from models.lm.trace.config import FL_TrACEConfig
from models.lm.trace.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.lm.trace.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _TrACEBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.attr_d_init = attr_d_init
        self.last_loss_branch = ""
        self.last_l2_loss = float("nan")
        self.last_ce_loss = float("nan")
        self.last_attr_loss = float("nan")
        self.last_attr_rho = float("nan")
        # d 是停梯度缓冲，不是网络参数；valid=0 时 L_attr 不进损失。
        self.register_buffer("attr_d", torch.zeros(text_encoder_dim))
        self.register_buffer("attr_d_valid", torch.zeros(()))
        self.register_buffer(
            "attr_d_last_opt", torch.zeros((), dtype=torch.long),
        )
        self._load_attr_d_init(attr_d_init)
    ...
    def _load_attr_d_init(self, attr_d_init: str | None) -> None:
        """可选预载 ``d``（短 FT / 冒烟）；``random`` 仅用于不跑 Alg.1 的路径。"""
        if attr_d_init is None:
            return
        raw = str(attr_d_init).strip()
        if raw in ("", "null", "None"):
            return
        if raw.lower() == "random":
            g = torch.Generator()
            g.manual_seed(0)
            d = torch.randn(self.text_encoder_dim, generator=g)
            d = d / (d.norm() + 1e-8)
            self.attr_d.copy_(d)
            self.attr_d_valid.fill_(1.0)
            return
        from models.lm.trace.ace import load_ace_direction

        d = load_ace_direction(
            raw,
            device=self.attr_d.device,
            dtype=self.attr_d.dtype,
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        # 只在去噪行、且 d 已有效时罚 +d 半轴；d 无梯度。
        denoise_w = (1.0 - decoder_step_active) * self.attr_d_valid.to(
            dtype=decoder_step_active.dtype,
        )
        u_theta = x_pred.mean(dim=1)
        d_vec = self.attr_d.detach().to(dtype=u_theta.dtype)
        attr_per = F.relu((u_theta * d_vec.view(1, -1)).sum(dim=-1)) ** 2
        attr_denom = denoise_w.sum()
        attr_loss = torch.where(
            attr_denom > 0,
            (attr_per * denoise_w).sum() / attr_denom.clamp(min=1.0),
            torch.zeros((), device=x_pred.device, dtype=x_pred.dtype),
        )
        self.last_attr_loss = torch.where(
            attr_denom > 0,
            attr_loss.detach(),
            torch.full(
                (), float("nan"), device=x_pred.device, dtype=x_pred.dtype,
            ),
        )
        return loss + (self.attr_weight * attr_loss)
    ...
    def on_optimizer_step(self, **ctx: Any) -> None:
        ...
        from models.lm.trace.track import maybe_refresh_attr_d
...
class FL_TrACEModel(FL_PreTrainedModel):
    config_class = FL_TrACEConfig

    def __init__(self, config: FL_TrACEConfig) -> None:
        super().__init__(config)
        self.backbone = _TrACEBackbone(**config.backbone_kwargs())
...
def build_model_from_config(config: FL_TrACEConfig) -> FL_TrACEModel:
    ...
    return FL_TrACEModel(config)


def build_model(cfg: dict) -> FL_TrACEModel:
    data, sampling = split_model_cfg(cfg)
    layout = token_layout_from_cfg(data)
    data.pop("tokenizer", None)
    config = FL_TrACEConfig(**data)
