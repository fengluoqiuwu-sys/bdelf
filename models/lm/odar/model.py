"""ODAR (Orthogonal Dual-Axis Repair; ELF backbone + DMA-H) — continuous diffusion LM with SC-CFG.
...
from models.lm.elf.ace import apply_ace_steer, resolve_ace_steering
from models.lm.odar.config import FL_ODARConfig
from models.lm.odar.dma import commit_mse, round_trip_st
from models.lm.odar.layers import (
    BottleneckTextProj,
    ODARBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.lm.odar.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _ODARBackbone(nn.Module):
    """ODAR DiT backbone with dual-branch (denoise MSE / decode CE) training."""
    ...
    def _token_embed_weight(self) -> torch.Tensor:
        """冻结 T5 input embedding 表 ``(V, C)``，供 DMA round-trip 查表。"""
        encoder = self._ensure_encoder()
        return encoder.model.get_input_embeddings().weight
    ...
    def _apply_dma(
        ...
        *,
        ...
        """对预测态做 DMA-H（``t >= dma_t0``）；``logits`` 须同一次 decode-head 输出。"""
        # 标量 t 早退：纯 Python，避免无谓查表与 GPU sync
        if isinstance(t, (int, float)) and float(t) < float(self.dma_t0):
            return x
        ...
        *,
        ...
        if self.self_cond_prob > 0:
            ...
            with torch.no_grad():
                ...
                x_init, logits_init = self.net_forward(
                    z_sc0, t, decoder_step_active=False, deterministic=True,
                )
                assert logits_init is not None
                # DMA-H：硬化 SC 反馈（仅 t >= dma_t0）；用同前向 decode logits
                x_init = self._apply_dma(x_init, t, logits=logits_init)
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        if self.num_self_cond_cfg_tokens > 0:
            ...
            # v 用原始 x；仅喂给下一步的反馈做 DMA（与 generate 语义一致）。
            with torch.no_grad():
            ...
            with torch.no_grad():
                x_fb = self._apply_dma(
                    x_init, t, logits=logits_init,
                    extra_gate=(1.0 - decoder_mask_b11),
                )
                x_uncond = x_fb.detach()
            ...
            # Pre-SC-CFG：单次 stop-grad self-cond，无 scale tokens。
            use_sc = (
                (torch.rand((bsz,), device=device, dtype=dtype) < self.self_cond_prob)
                .reshape(-1, 1, 1)
                .to(dtype=dtype)
    ...
    def _forward_self_cond(
        ...
        """One SC-CFG-aware denoiser forward; returns (v, x_pred, decode_logits)."""
        if self.self_cond_prob > 0:
    ...
    def _forward_sample(
        ...
        Returns ``(v, x_pred, logits)``；logits 供 DMA 量化（与 x 同前向）。
        """
        bsz = z.shape[0]
        ...
        v_uncond = x_uncond = logits_uncond = None
        if need_uncond:
            v_uncond, x_uncond, logits_uncond = self._forward_self_cond(
                z, t_batch, None, None,
            )
            if w == 0.0 or x_pred_prev is None:
                return v_uncond, x_uncond, logits_uncond

        v_cond, x_cond, logits_cond = self._forward_self_cond(
            z, t_batch, x_pred_prev, None,
        )
        if w == 1.0:
            return v_cond, x_cond, logits_cond
        assert v_uncond is not None and x_uncond is not None
        v_out = v_uncond + w * (v_cond - v_uncond)
        x_out = x_uncond + w * (x_cond - x_uncond)
        # 外推 x 时用 cond 读出头做 DMA 量化（CFG 主信号）
        return v_out, x_out, logits_cond
    ...
    def _ode_step(
        ...
        v, x_pred, logits = self._forward_sample(
            z, t, x_pred_prev, self_cond_cfg_scale=self_cond_cfg_scale,
        )
        return z + (t_next - t) * v, x_pred, logits
    ...
    def _sde_step(
        ...
        return z_back + (t_next - t_back) * v, x_pred, logits
    ...
    def _decode_tokens(
        ...
        *,
        ...
        if not torch.isfinite(logits).all():
            raise RuntimeError("ODAR decode produced non-finite logits")
        if temperature <= 0:
    ...
    def generate(
        ...
        *,
        ...
        del bos_token_id, prefix_tokens  # unconditional ODAR
        cfg = sampling_cfg or {}
        if seqlen is None:
        ...
        for i in range(t_steps.numel() - 2):
            ...
            if method == "sde":
                z, x_pred, step_logits = self._sde_step(
                    z, t, t_next, x_pred, sde_gamma,
                    self_cond_cfg_scale=self_cond_cfg_scale,
                )
            elif method == "ode":
                z, x_pred, step_logits = self._ode_step(
                    z, t, t_next, x_pred,
                    self_cond_cfg_scale=self_cond_cfg_scale,
                )
            else:
        ...
        z, x_pred, _ = self._ode_step(
            z, t, t_next, x_pred,
            self_cond_cfg_scale=self_cond_cfg_scale,
        )
        nfe += 1
        if not torch.isfinite(z).all():
            raise RuntimeError("ODAR sampling produced non-finite latents")
...
class FL_ODARModel(FL_PreTrainedModel):
    config_class = FL_ODARConfig

    def __init__(self, config: FL_ODARConfig) -> None:
        super().__init__(config)
        self.backbone = _ODARBackbone(**config.backbone_kwargs())
...
def build_model_from_config(config: FL_ODARConfig) -> FL_ODARModel:
