...
Shared kernel under ``models.lm.elf_core``; family packages re-export this module.
"""
...
def model_hash_from_checkpoint(ckpt_path: str | Path) -> str | None:
    """从 checkpoint run 路径解析训练 hash（旧/新布局均可）。"""
    ...
    try:
        ...
        rel_parts = rel.parts
    # 旧 {variant}/{model}/{hash}/… 或新 {variant}/{kind}/{model}/{hash}/…
    if len(rel_parts) < 3:
        return None
    if len(rel_parts) >= 4 and rel_parts[1] in ("lm", "latent"):
        return str(rel_parts[3])
...
def estimate_ace_direction(
    ...
    *,
    ...
    while done < n:
        cur = min(bs, n - done)
        # 与 generate_tokens 类似的分块种子，避免整批共用同一噪声
        ...
        for i in range(cur):
            text = tok.decode(tokens[i].detach().cpu().tolist(), skip_special_tokens=True)
            feats.append(u[i].detach().float().cpu())
            reps.append(seq_rep_4(text))
        done += cur
        _log(f"estimate progress {done}/{n}")

    S = torch.stack(feats, dim=0)
    r = torch.tensor(reps, dtype=torch.float32)
    order = torch.argsort(r)
    t = max(1, len(order) // 3)
    d = S[order[-t:]].mean(0) - S[order[:t]].mean(0)
    d = d / (d.norm() + 1e-8)
    _log(
        f"estimate done tertile={t} "
        f"rep_lo={float(r[order[:t]].mean()):.4f} "
        f"rep_hi={float(r[order[-t:]].mean()):.4f} "
        f"|d|={float(d.norm()):.4f}"
    )
    return d
