"""Frozen T5 encoder used as ELF-family continuous embedding source (training only).

Shared by ``models.lm.elf`` and sibling packages via thin re-export facades.
"""
...
def ensure_t5_encoder_cached(
    ...
    *,
    ...
    ``log_tag`` only affects print prefixes (default ``elf`` matches historical
    ELF / late_ce / lexce / posbeta / trace / denoiser_chart copies).
    """
    ...
    print(
        f"[{log_tag}] Local T5 encoder not found at {local_dir}; "
    ...
    print(f"[{log_tag}] T5 encoder ready at {local_dir}")
    return str(local_dir)
...
class T5Encoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if pretrained:
            local_dir = ensure_t5_encoder_cached(
                config.model_name, log_tag=log_tag,
            )
