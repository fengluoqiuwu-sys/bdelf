"""LoopSC — ELF 复制体：训练展开采样 SC 闭环，主损失仍是对 x0 的 FM。

骨干与推理同 ELF（arXiv:2605.10938）。训练期把采样自条件环展开
``sc_unroll_steps`` 步（默认 2）：上一拍完整 ``x_hat`` 作为下一拍 SC
（不减均值、不减 λd），ODE 状态默认 detach，梯度只走 SC 半边。
``self_cond_prob`` 仍为 0.5；推理仍是带 ``w`` 的单次前向，ACE=off。
...
from models.lm.loopsc.ace import (
    ace_step_active,
    apply_ace_steer,
    parse_ace_step_range,
    resolve_ace_steering,
)
from models.lm.loopsc.config import FL_LoopSCConfig
from models.lm.loopsc.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.lm.loopsc.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _LoopSCBackbone(nn.Module):
    """LoopSC DiT 骨干：denoise MSE / decode CE，训练展开闭环 SC。"""
    ...
    def __init__(
        ...
        sc_unroll_steps: int = 2,
        sc_unroll_bptt: bool = True,
        sc_unroll_detach_z: bool = True,
    ) -> None:
        super().__init__()
        ...
        self.sc_unroll_steps = int(sc_unroll_steps)
        if self.sc_unroll_steps < 1:
            raise ValueError("sc_unroll_steps must be >= 1")
        self.sc_unroll_bptt = bool(sc_unroll_bptt)
        self.sc_unroll_detach_z = bool(sc_unroll_detach_z)
    ...
    def _unroll_sc_chain(
        self,
        x0: torch.Tensor,
        noise: torch.Tensor,
        t_last: torch.Tensor,
        self_cond_cfg: torch.Tensor | None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """从 ``t_last-(K-1)Δt`` 沿模型 ODE 走到 ``t_last``；SC 为上一拍完整 ``x_hat``。

        返回 ``(z_last, sc_prev)``。默认把 z detach，只让梯度走 SC 半边。
        Δt=1/32，与推理网格同量级。``K=1`` 时不应调用。
        """
        k_steps = int(self.sc_unroll_steps)
        if k_steps < 2:
            raise ValueError("sc_unroll_steps must be >= 2 to unroll")
        offsets = torch.arange(
            k_steps - 1, -1, -1, device=t_last.device, dtype=t_last.dtype,
        )
        t_chain = torch.clamp(t_last.unsqueeze(1) - offsets * (1.0 / 32.0), min=0.0)
        t0 = t_chain[:, 0]
        z = t0.reshape(-1, 1, 1) * x0 + (1.0 - t0.reshape(-1, 1, 1)) * noise
        sc: torch.Tensor | None = None
        bptt = bool(self.sc_unroll_bptt)
        detach_z = bool(self.sc_unroll_detach_z)
        ctx = nullcontext() if bptt else torch.no_grad()
        with ctx:
            for i in range(k_steps - 1):
                t_i = t_chain[:, i]
                t_n = t_chain[:, i + 1]
                v, x_pred = self._forward_self_cond(z, t_i, sc, self_cond_cfg)
                dt = (t_n - t_i).reshape(-1, 1, 1)
                z_next = z + dt * v
                z = z_next.detach() if detach_z else z_next
                sc = x_pred if bptt else x_pred.detach()
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        """denoise/decode 混合；SC 行展开采样闭环，主损失仍是对 x0 的 FM。"""
        bsz, seq_len, _ = x0.shape
        device = x0.device
        dtype = x0.dtype
        ...
        use_sc = torch.zeros((bsz, 1, 1), device=device, dtype=dtype)
        if self.self_cond_prob > 0:
            use_sc = (
        ...
        z_denoise = denoiser_z
        sc_from_chain: torch.Tensor | None = None
        do_unroll = torch.zeros((bsz, 1, 1), dtype=torch.bool, device=device)
        if int(self.sc_unroll_steps) >= 2 and self.self_cond_prob > 0:
            z_u, sc_u = self._unroll_sc_chain(x0, noise, t, self_cond_cfg)
            do_unroll = (use_sc > 0) & (decoder_mask_b11 == 0)
            z_denoise = torch.where(do_unroll, z_u, denoiser_z)
            sc_from_chain = sc_u

        z_mixed = decoder_mask_b11 * decoder_z + (1.0 - decoder_mask_b11) * z_denoise
        v_target = (x0 - z_denoise) / torch.clamp(1.0 - t_exp, min=self.t_eps)

        if self.num_self_cond_cfg_tokens > 0:
            # SC-CFG 老师仍在最后一拍 (z, t) 上算；学生 SC 来自上一采样拍。
            with torch.no_grad():
                z_sc0 = torch.cat([z_denoise, torch.zeros_like(z_denoise)], dim=-1)
                x_init, _ = self.net_forward(
                    z_sc0, t, decoder_step_active=False, deterministic=True,
                    self_cond_cfg_scale=self_cond_cfg,
                )
            v_uncond = self._x_to_v(x_init, z_denoise, t)
            x_uncond = x_init.detach()
            with torch.no_grad():
                z_sc1 = torch.cat([z_denoise, x_uncond], dim=-1)
                x_cond, _ = self.net_forward(
                    z_sc1, t, decoder_step_active=False, deterministic=True,
                    self_cond_cfg_scale=self_cond_cfg,
                )
            v_cond = self._x_to_v(x_cond, z_denoise, t)
            ...
            sc_same_t = x_uncond * use_sc
            if sc_from_chain is not None:
                sc_half = torch.where(do_unroll, sc_from_chain, sc_same_t)
            else:
                sc_half = sc_same_t
            sc_half = sc_half * (1.0 - decoder_mask_b11)
            ...
            sc_same_t = x_init.detach() * use_sc
            if sc_from_chain is not None:
                sc_half = torch.where(do_unroll, sc_from_chain, sc_same_t)
            else:
                sc_half = sc_same_t
        ...
        v_pred = self._x_to_v(x_pred, z_denoise, t)
        l2_per_token = ((v_pred - v_target) ** 2).mean(dim=-1)
    ...
    def _decode_tokens(
        ...
        *,
        ...
        if not torch.isfinite(logits).all():
            raise RuntimeError("LoopSC decode produced non-finite logits")
        if temperature <= 0:
    ...
    def generate(
        ...
        *,
        ...
        if not torch.isfinite(z).all():
            raise RuntimeError("LoopSC sampling produced non-finite latents")
...
class FL_LoopSCModel(FL_PreTrainedModel):
    config_class = FL_LoopSCConfig

    def __init__(self, config: FL_LoopSCConfig) -> None:
        super().__init__(config)
        self.backbone = _LoopSCBackbone(**config.backbone_kwargs())
...
def build_model_from_config(config: FL_LoopSCConfig) -> FL_LoopSCModel:
    ...
    return FL_LoopSCModel(config)


def build_model(cfg: dict) -> FL_LoopSCModel:
    data, sampling = split_model_cfg(cfg)
    layout = token_layout_from_cfg(data)
    data.pop("tokenizer", None)
    config = FL_LoopSCConfig(**data)
