...
class _RelfBackbone(nn.Module):
    ...
    def __init__(self, config: FL_RelfConfig, bundle: LatentBundle) -> None:
        ...
        if self.step_size < 1:
            raise ValueError(f"relf step_size S 须为正，收到 {self.step_size}")
        if self.window_size % self.step_size != 0:
            raise ValueError(
                f"relf 要求 window_size 能被 step_size 整除，"
                f"收到 W={self.window_size} S={self.step_size}"
            )
        # 推理 T=W/S，由窗几何推出，不是训练超参。
        self.infer_t = self.window_size // self.step_size
        check_time_step(self.infer_t)
        ...
        if str(config.train_t_schedule).strip().lower() != "block":
            raise ValueError(
                f"relf train_t_schedule 锁死 block，收到 {config.train_t_schedule!r}"
            )
        ...
        bundle.require_vae_dec(reason="relf exit locked to VAE-dec")
        ...
        # F[k]=L_{T-1-⌊k/S⌋}：窗左是 L_{T-1}，不是 1-ε；L_T 只作末次 Euler 终点。
        f_k = torch.stack(
            [levels[self.infer_t - 1 - (k // self.step_size)] for k in range(self.window_size)]
        )
        self.register_buffer("F", f_k, persistent=False)
        self._ladder_cap = float(levels[-1].item())  # L_T=1-ε，勿用来标 decode CE
        self._gen_mask_cache: dict[tuple[int, torch.device], torch.Tensor] = {}

        self.last_l2_loss: torch.Tensor | float = float("nan")
        self.last_ce_loss: torch.Tensor | float = float("nan")  # 兼容旧日志列；恒为 nan
    ...
    def prefill_left_kv(
        ...
        *,
        ...
        mask = group_causal_mask(left_len, 1, device=x.device)
        return self.denoiser.prefill_left(
            x, attn_mask=mask, positions=positions, known_max=known_max,
        )
    ...
    def extend_left_kv(
        ...
        *,
        ...
        if cache is None or cache.left_len == 0:
            return self.prefill_left_kv(h_new, positions, known_max=known_max)
        x = h_new
        if self.sc_proj is not None:
            x = self.sc_proj(torch.cat([x, torch.zeros_like(x)], dim=-1))
        return self.denoiser.extend_left(
            x, cache, positions=positions, left_group=1, known_max=known_max,
        )
    ...
    def _pack_forward(
        ...
        *,
        ...
        # 左段先留在 X 空间；self-left 替换后再茎。
        h_left_x, drop_left = maybe_drop_left(
        ...
        t_block = sample_logit_normal_t(
            (bsz, n_win, w_sz // step),
            float(self.config.denoiser_p_mean),
            float(self.config.denoiser_p_std),
            device=device,
            dtype=dtype,
        )
        t_w = t_block.repeat_interleave(step, dim=-1)
        t_w = torch.where(
            in_win & (~known),
            t_w,
            torch.ones(bsz, n_win, w_sz, device=device, dtype=dtype),
        )
        unknown = in_win & (~known)
        m_w = torch.zeros(bsz, n_win, w_sz, device=device, dtype=torch.long)
        m_w = torch.where(unknown, torch.full_like(m_w, _MODE_DENOISE), m_w)
        ...
        # self-left：p 在图外算好；p>0 时整 batch 跑 2L G，再用 mask 写回。
        p_left = (
            float(self._self_left_p) if (self.training and compute_loss) else 0.0
        )
        if p_left > 0.0 and left_len > 0:
            use_self = torch.rand(bsz, device=device) < p_left
            t_last = t_w.new_full((), 1.0 - float(self.config.t_clean_eps))
            ...
            with torch.no_grad():
                h_gt = pack_2l(self._stem(h_ctx[:, :seq_len].detach()), self._stem(z_c_right))
                t_c_all = torch.cat([t_left, t_c_right], dim=1)
                m_c_all = torch.cat([m_left, m_c_right], dim=1)
            ...
            buf.scatter_(
                1,
                idx_w.reshape(bsz, right_len, 1).expand(bsz, right_len, x_dim).long(),
                src,
            )
            sl = use_self[:, None, None]
            h_left_x = torch.where(sl, buf[:, :seq_len], h_left_x)
        ...
        if left_len == 0:
            ...
            # 整 batch teacher；勿 bool(guided.any())，各 rank 抽样独立会分叉死锁。
            if need_teacher:
                with torch.no_grad():
                    x_u_r, _ = self._split_g(self._run_g(
                        h_right, t_right, w_in, m_right, torch.zeros_like(h_right),
            ...
            if need_teacher:
                with torch.no_grad():
                    x_u_r, _ = self._split_g(self._run_g_right(
                        h_right, t_right, w_in, m_right, torch.zeros_like(h_right),
                        left_kv, attn_mask=attn, positions=pos_right,
                    ...
                    x_c_r, _ = self._split_g(self._run_g_right(
                        h_right, t_right, w_in, m_right, sc_r_cond, left_kv,
                        attn_mask=attn, positions=pos_right, flex_block_mask=flex_mask,
                    ))
                    v_c = x_to_v(x_c_r, z_right, t_right, vel_eps)
                assert sc_stu_r is not None
        ...
        self.last_ce_loss = mse.new_full((), float("nan"))
        loss = float(self.config.lambda_mse) * mse
        return loss, mse
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # last_ce_loss 已在 _pack_forward 写成 nan
    ...
    def _slot_next_t(self, t: torch.Tensor) -> torch.Tensor:
        ...
        # 找当前档 i 使 L_i ≈ t；下一档 min(i+1, T)，L_T 为末次 Euler 终点。
        dist = (t.unsqueeze(-1) - levels.view(*([1] * t.ndim), -1)).abs()
        idx = dist.argmin(dim=-1)
        nxt = (idx + 1).clamp(max=int(self.infer_t))
        return levels[nxt]
    ...
    def _churn_eval_state(
        ...
        *,
        ...
        仅 denoise 且非末流槽 churn；已知 / 末流保持原 ``(z, t)``。
        """
        if method != "sde":
            return z, t
        h = t_next - t
    ...
    def rolling_generate(
        ...
        *,
        ...
        if "num_sampling_steps" in cfg:
            raise ValueError(
                f"relf 禁止 num_sampling_steps（T=W/S={self.infer_t} 由窗几何推出），"
                f"收到 {cfg['num_sampling_steps']}"
            )
        ...
        # 前缀词用于 BOS 几何与已知余数；commit_x0hat=false 时循环内读词并可按 EOS 早停。
        tok_like = prefix if prefix.size(1) > 0 else prefix.new_zeros(num_samples, 1)
        tok_sb = SeqBuf(known_max=seqlen, seq_dim=1, like=tok_like, fill=pad_id)
        if prefix.size(1) > 0:
            tok_sb.replace(prefix)
        tokens = tok_sb.view()
        prefix_len = int(tokens.size(1))
        nfe = 0
        ...
        def _one_g(
            ...
            *,
            ...
            if use_cache:
                ...
                left_xh = left_kv.x_hat_filled
                if left_xh is None:
                    raise RuntimeError("left_kv.x_hat 不能为空")
                x_hat = torch.cat([left_xh, x_right], dim=1)
                hidden = None
                left_h = left_kv.hidden_filled
                if hidden_r is not None and left_h is not None:
                    hidden = torch.cat([left_h, hidden_r], dim=1)
                nfe += 1
                return x_hat, hidden
        ...
        def _fields_for_u(u: int, tok: torch.Tensor, *, target_len: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
            ...
            # 窗内未知一律 denoise；不再用 cap 标 MODE_DECODE。
            m_w = torch.where(
                in_win,
        ...
        def _emit_mask(in_win: torch.Tensor, known: torch.Tensor | None = None) -> torch.Tensor:
            """末次 Euler 后提交：最左档（F[0]=L_{T-1}）且仍在窗的未知槽。"""
            f = self.F.to(device=in_win.device, dtype=dtype)
            left_rung = (f - f[0]).abs() < 1e-6
            emit = in_win & left_rung.view(1, -1).expand_as(in_win)
            if known is not None:
                emit = emit & (~known)
            return emit

        h_like = torch.zeros(num_samples, 1, self.n_embd, device=device, dtype=dtype)
        h_sb = SeqBuf(known_max=seqlen, seq_dim=1, like=h_like)
        x_like = torch.zeros(num_samples, 1, self.latent_dim, device=device, dtype=dtype)
        x_sb = SeqBuf(known_max=seqlen, seq_dim=1, like=x_like)

        def _set_h_left(src: torch.Tensor | None) -> torch.Tensor:
            if src is None or int(src.size(1)) == 0:
                h_sb.filled = 0
                return h_sb.view()
            return h_sb.replace(src)

        def _cat_h_left(pref: torch.Tensor, new: torch.Tensor) -> torch.Tensor:
            n_pref = int(pref.size(1))
            same_pref = (
                n_pref > 0
                and h_sb.filled >= n_pref
                and pref.data_ptr() == h_sb.buf.data_ptr()
            )
            if n_pref == 0:
                return _set_h_left(new)
            if same_pref:
                h_sb.filled = n_pref
            else:
                h_sb.replace(pref)
            if int(new.size(1)) == 0:
                return h_sb.view()
            return h_sb.append(new)

        if prefix_len > 0:
            x_pref = _encode_mapped(tokens, self.config.x0_source, sample=False)
            x_sb.replace(x_pref)
            h_left_cache: torch.Tensor | None = _set_h_left(self._stem(x_pref))
        else:
            h_left_cache = _set_h_left(None)
        ...
        def _ensure_left_kv(
            ...
            if left_kv is not None and left_kv_key == key:
                return left_kv
            left_kv = self.prefill_left_kv(h_left, pos_left, known_max=seqlen)
            left_kv_key = key
            return left_kv
        ...
        def _append_committed(
            add: torch.Tensor, *, left_pref: torch.Tensor | None = None,
        ) -> None:
            nonlocal h_left_cache
            x_sb.append(add)
            h_new = self._stem(add)
            if left_pref is None:
                pref = h_sb.view() if h_sb.filled > 0 else add.new_zeros(
                    add.size(0), 0, self.n_embd,
                )
            else:
                pref = left_pref
            h_left_cache = _cat_h_left(pref, h_new)
            n_old = int(h_left_cache.size(1)) - int(add.size(1))
            pos_new = torch.arange(n_old, int(h_left_cache.size(1)), device=device)
            _note_left_kv(
                h_left_cache,
                self.extend_left_kv(left_kv, h_new, pos_new, known_max=seqlen),
            )

        def _commit_emit(
            x_right: torch.Tensor, emit: torch.Tensor, n_write: int,
        ) -> int:
            keep = emit
            ...
            order = torch.where(
                keep, idx, torch.full_like(idx, w_sz),
            ).sort(dim=1).values
            n_real = int((order < w_sz).sum(dim=1).max().item())
            n_out = min(int(n_write), n_real, seqlen - x_sb.filled)
            if n_out < 1:
                return 0
            take = order[:, :n_out]
            valid = take < w_sz
            take3 = take.clamp(max=w_sz - 1).unsqueeze(-1).expand(
                -1, -1, x_right.size(-1),
            )
            gathered = x_right.gather(1, take3)
            add = torch.where(
                valid.unsqueeze(-1), gathered, torch.zeros_like(gathered),
            )
            _append_committed(add)
            return n_out
        ...
        def _append_decoded(
            ...
            dec: torch.Tensor,
            alive_row: torch.Tensor,
            *,
            n_write: int,
        ) -> tuple[torch.Tensor, torch.Tensor, int, torch.Tensor]:
            """追加 VAE-dec 读出的 token；返回 ``(tokens, hit, n_out, valid)``。"""
            ...
            keep = dec & (prev_eos == 0) & alive_row[:, None] & (~already)[:, None]
            ...
            return tok_sb.append(add), hit, n_out, valid

        def _exit_tok(x_hat: torch.Tensor, left_len: int) -> torch.Tensor:
        ...
        while True:
            L = int(x_sb.filled) if commit_x0 else int(tok_sb.filled)
            if L >= seqlen:
                break
            if not commit_x0:
                if not bool(alive.any().item()):
                    break
                if tokens.size(1) > 0 and bool((tokens == eos_id).any(dim=1).all()):
                    break
            r = L % step
            g = (L // step) * step

            if L == 0:
                # preroll：T 次 denoise Euler；末 hop 提交或 VAE-dec。
                ...
                for hop in range(self.infer_t):
                    k_bos = step * (self.infer_t - hop - 1)
                    u = -k_bos
                    ...
                    if hop < self.infer_t - 1:
                        ...
                        if commit_x0:
                            n_out = _commit_emit(
                                x_right, _emit_mask(in_win), step,
                            )
                        else:
                            ...
                            tokens, hit, n_out, _valid = _append_decoded(
                                tokens, sampled, _emit_mask(in_win), alive,
                                n_write=step,
                            )
                            tokens = tok_sb.replace(
                                pad_after_first_eos(tokens, eos_id, pad_id),
                            )
                            alive = alive & ~hit
                            h_left_cache = _set_h_left(_commit_left(tokens))
                            _invalidate_left_kv()
                        z_carry = torch.cat(
                            [
                                z_win[:, step:],
                ...
                grown = int(x_sb.filled) if commit_x0 else int(tok_sb.filled)
                if grown < 1:
                    break
                continue

            if L > 0 and (r > 0 or z_carry is None):
                # 余数爬梯：T 次 denoise Euler；末 hop 提交 x_hat 或 VAE-dec 再 encode。
                # 与训练单帧 (h,r) 同构。完整 pop [0,g) 进 KV。
                ...
                if commit_x0:
                    h_x0_pref = x_sb.view() if x_sb.filled > 0 else _encode_x0(tokens)
                else:
                    h_x0_pref = _encode_x0(tokens)
                hop0 = True
                for hop in range(self.infer_t):
                    k0 = step * (self.infer_t - hop - 1)
                    u = g - k0
                    t_w, m_w, in_win = _fields_for_u(u, tokens, target_len=seqlen)
                    ...
                    past_end = g + step >= seqlen
                    if not commit_x0:
                        past_end = past_end or bool((tokens == eos_id).any(dim=1).all())
                    extra = (
                        torch.zeros(
                            num_samples, step, self.latent_dim, device=device, dtype=dtype,
                        )
                    ...
                    if hop < self.infer_t - 1:
                        ...
                        # 末 hop 已 Euler 到 L_T。
                        if commit_x0:
                            n_write = min(step - r, seqlen - L)
                            n_pop = int(r) + int(n_write)
                            ...
                            if n_pop > 0:
                                ...
                                _append_committed(pop, left_pref=left_pref)
                            else:
                                h_left_cache = _set_h_left(left_pref)
                        else:
                            right_n = int(x_hat.size(1)) - int(h_left.size(1))
                            logits = self._exit_logits(
                            ...
                            tokens, hit, _n_out, _valid = _append_decoded(
                                tokens, sampled, _emit_mask(in_win, known), alive,
                                n_write=step - r,
                            )
                            tokens = tok_sb.replace(
                                pad_after_first_eos(tokens, eos_id, pad_id),
            ...
            # freeroll：r=0。窗从当前长度起铺 F；一次 Euler 后提交或 VAE-dec。
            u = L
            t_w, m_w, in_win = _fields_for_u(u, tokens, target_len=seqlen)
            m_w = torch.where(in_win, m_w, torch.zeros_like(m_w))
            if z_carry is None:
            ...
            if commit_x0:
                n_out = _commit_emit(x_right, _emit_mask(in_win), step)
                if n_out < 1:
                ...
                tokens, hit, n_out, _valid = _append_decoded(
                    tokens, sampled, _emit_mask(in_win), alive, n_write=step,
                )
                tokens = tok_sb.replace(pad_after_first_eos(tokens, eos_id, pad_id))
                alive = alive & ~hit
                ...
                h_left_cache = _set_h_left(_commit_left(tokens))
                _invalidate_left_kv()
            past_end = u + step >= seqlen
            if not commit_x0:
                past_end = past_end or (not bool(alive.any().item()))
            extra = (
                torch.zeros(
                    num_samples, step, self.latent_dim, device=device, dtype=dtype,
                )
        ...
        if commit_x0:
            # 全文一次 VAE-dec；前缀词写回后按首个 EOS 填 PAD。
            dec_in = x_sb.view()
            if dec_in.size(1) < 1:
                out = torch.full(
            ...
            if prefix_len > 0:
                dummy = eos_id if int(eos_id) != int(pad_id) else int(pad_id) + 1
                dec_tok = torch.full(
                    (num_samples, int(dec_in.size(1))), dummy,
                    device=device, dtype=torch.long,
                )
                dec_tok[:, :prefix_len] = tokens[:, :prefix_len]
            logits = self._exit_logits(dec_in, tokens=dec_tok)
            sampled = self._sample_tokens(
                logits, temperature=temperature, top_k=top_k,
            )
            if sampled.size(1) < seqlen:
                pad = sampled.new_full((num_samples, seqlen - sampled.size(1)), pad_id)
                sampled = torch.cat([sampled, pad], dim=1)
            else:
                sampled = sampled[:, :seqlen]
            if prefix_len > 0:
                sampled = sampled.clone()
                sampled[:, :prefix_len] = tokens[:, :prefix_len]
            tokens = pad_after_first_eos(sampled, eos_id, pad_id)
            return tokens, nfe
        if tok_sb.filled < seqlen:
            pad = torch.full(
                (num_samples, seqlen - tok_sb.filled), pad_id,
                device=device, dtype=torch.long,
            )
            tokens = tok_sb.append(pad)
        else:
            tokens = tok_sb.view()[:, :seqlen]
        return tokens, nfe
