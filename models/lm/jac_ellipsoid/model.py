"""JacEllipsoid：ELF 复制体，decode / 采样改为 QDA 观测（不改 models/elf）。

骨架、双分支混损与 SC-CFG 同 ELF。相对 ELF 只换读出：
``p(x|z)`` 为冻 T5 表上的各向异性高斯判别（jacobian），或对照
softmax / 各向同性 / 同秩可学 Σ。中段 FM 仍为欧氏 MSE。
"""
...
from models.lm.jac_ellipsoid.ace import (
    ace_step_active,
    apply_ace_steer,
    parse_ace_step_range,
    resolve_ace_steering,
)
from models.lm.jac_ellipsoid.config import FL_JacEllipsoidConfig
from models.lm.jac_ellipsoid.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.lm.jac_ellipsoid.qda import (
    QDA_MODES,
    build_isotropic_tables,
    build_jacobian_tables,
    diag_from_factor,
    load_qda_tables_file,
    load_t5_codebook,
    logdet_from_factor,
    pad_tables_to_vocab,
    qda_logits,
...
from models.lm.jac_ellipsoid.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _ELFBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.qda_table_path = "" if qda_table_path is None else str(qda_table_path).strip()
        self.qda_decode_only = bool(qda_decode_only)
        self.freeze_dit = bool(freeze_dit)
        self.qda_diag = bool(qda_diag)
        if self.qda_decode_only:
            self.decoder_prob = 1.0
            self.mixed_branch_training = False
        if self.qda_mode not in QDA_MODES:
            raise ValueError(
                f"qda_mode must be one of {QDA_MODES}, got {qda_mode!r}"
            )
        ...
        self._init_qda_tables()
        self._apply_freeze_dit()

    def _init_qda_tables(self) -> None:
        """从 tables.pt 或冻 T5 表构造 QDA 缓冲；softmax 对照不建表。"""
        dim = int(self.text_encoder_dim)
        vocab = int(self.vocab_size)
        self.register_buffer(
            "qda_lam_t", torch.tensor(1.0, dtype=torch.float32), persistent=True,
        )
        if self.qda_mode == "softmax":
            self.qda_factor = nn.Parameter(
                torch.zeros(vocab, 0, dim), requires_grad=False,
            )
            self.register_buffer("qda_mu", torch.zeros(vocab, dim), persistent=True)
            self.register_buffer(
                "qda_log_pi",
                torch.full((vocab,), -torch.log(torch.tensor(float(max(vocab, 1))))),
                persistent=True,
            )
            self.register_buffer("qda_logdet", torch.zeros(vocab), persistent=True)
            self.register_buffer("qda_diag_w", torch.ones(vocab, dim), persistent=True)
            return

        mu_t5, unembed, _spec = load_t5_codebook(
        ...
        if mu_t5.shape[1] != dim:
            raise ValueError(
                f"T5 codebook dim {mu_t5.shape[1]} != text_encoder_dim {dim}"
            )

        source = "t5-svd"
        if self.qda_table_path:
            blob = load_qda_tables_file(self.qda_table_path)
            padded = pad_tables_to_vocab(
                blob, vocab=vocab, dim=dim, fallback_mu=mu_t5,
            )
            mu = padded["mu"]
            factor = padded["factor"]
            log_pi = padded["log_pi"]
            lam = float(padded["lam"])
            n_loaded = int(padded["n_loaded"])
            cons = padded["constructable"]
            coverage = (
                float(cons[:n_loaded].float().mean().item()) if n_loaded else 0.0
            )
            source = f"table:{self.qda_table_path}"
            print(
                f"[jac_ellipsoid] 加载 QDA 表 {self.qda_table_path} "
                f"n_loaded={n_loaded}/{vocab} lam={lam:.4g} "
                f"rank={int(factor.shape[1])}",
                flush=True,
            )
            if self.qda_mode == "isotropic":
                tables = build_isotropic_tables(mu, log_pi=log_pi)
                mu, factor, log_pi, lam = (
                    tables.mu, tables.factor, tables.log_pi, tables.lam,
                )
                coverage = tables.coverage
                source = f"isotropic({source})"
            else:
                self.qda_rank = int(factor.shape[1])
        elif self.qda_mode == "isotropic":
            tables = build_isotropic_tables(mu_t5)
            mu, factor, log_pi, lam = (
                tables.mu, tables.factor, tables.log_pi, tables.lam,
            )
            coverage = tables.coverage
            ...
            tables = build_jacobian_tables(
                mu_t5,
                unembed,
                top_m=self.qda_top_m,
                rank=self.qda_rank,
                lambda_scale=self.qda_lambda_scale,
            )
            mu, factor, log_pi, lam = (
                tables.mu, tables.factor, tables.log_pi, tables.lam,
            )
            coverage = tables.coverage
            print(
                f"[jac_ellipsoid] QDA coverage={tables.coverage:.3f} "
                f"(floor={tables.meta.get('coverage_floor')}) "
                f"lam={tables.lam:.4g} rank={self.qda_rank}",
                flush=True,
            )

        learned = self.qda_mode == "learned"
        self.qda_lam_t.fill_(float(lam))
        self.register_buffer("qda_mu", mu.contiguous(), persistent=True)
        self.register_buffer("qda_log_pi", log_pi.contiguous(), persistent=True)
        logdet = logdet_from_factor(factor, float(lam), dim)
        self.register_buffer("qda_logdet", logdet.contiguous(), persistent=True)
        self.register_buffer(
            "qda_diag_w", diag_from_factor(factor, float(lam)).contiguous(), persistent=True,
        )
        self.qda_factor = nn.Parameter(factor.clone(), requires_grad=learned)
        self._qda_source = source
        self._qda_coverage = coverage

    def _apply_freeze_dit(self) -> None:
        """M0/M1 短训：冻 DiT / 时间前缀 / text_proj，只训读出头。"""
        if not self.freeze_dit:
            return
        frozen = [self.self_cond_proj, self.text_proj, self.t_embedder, self.blocks]
        if self.self_cond_cfg_embedder is not None:
            frozen.append(self.self_cond_cfg_embedder)
        for mod in frozen:
        ...
        n = self.trainable_parameter_count()
        print(f"[jac_ellipsoid] freeze_dit=True 可训参数={n}", flush=True)

    @property
    def qda_lam(self) -> float:
        return float(self.qda_lam_t.item())

    def _readout_logits(
        self,
        x_pred: torch.Tensor,
        unembed_logits: torch.Tensor | None,
    ) -> torch.Tensor:
        """Decode 读出：softmax 走原 unembed，其余走 QDA(x_pred)。"""
        if self.qda_mode == "softmax":
            if unembed_logits is None:
                raise RuntimeError("softmax 读出需要 unembed logits")
            return unembed_logits
        if self.qda_diag:
            if self.qda_mode == "learned":
                diag_w = diag_from_factor(self.qda_factor.float(), self.qda_lam_t)
            else:
                diag_w = self.qda_diag_w.float()
            return qda_logits_diag(
                x_pred.float(),
                mu=self.qda_mu.float(),
                diag_w=diag_w,
                log_pi=self.qda_log_pi.float(),
            ).to(dtype=x_pred.dtype)
        factor = self.qda_factor
        if self.qda_mode == "learned":
            logdet = logdet_from_factor(
                factor, self.qda_lam_t, self.text_encoder_dim,
            )
        else:
            logdet = self.qda_logdet
        return qda_logits(
            x_pred.float(),
            mu=self.qda_mu.float(),
            factor=factor.float(),
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            if decoder_step_active is not None:
                if self.qda_mode == "softmax":
                    ...
                    # QDA 读出走 x_pred；跳过 [B,S,V] unembed GEMM。
                    decoder_logits = None
    ...
    def _touch_unembed_head(self, loss: torch.Tensor) -> torch.Tensor:
        """QDA 路径跳过 unembed / 空 decode 行时，把读出参数留在图里（DDP）。"""
        if self.qda_mode == "softmax":
            return loss
        return loss + 0.0 * (
    ...
    def _ce_per_token(
        ...
        *,
        ...
        """逐 token NLL。``row_mask`` 为 ``[B]`` 时只对那些行跑读出。"""
        if row_mask is None:
            logits = self._readout_logits(x_pred, unembed_logits)
            log_probs = F.log_softmax(logits.float(), dim=-1)
            return -log_probs.gather(-1, tokens.unsqueeze(-1)).squeeze(-1)
        bsz, seq_len = tokens.shape
        out = x_pred.new_zeros(bsz, seq_len)
        x_d = x_pred[row_mask]
        tok_d = tokens[row_mask]
        u_d = None if unembed_logits is None else unembed_logits[row_mask]
        logits = self._readout_logits(x_d, u_d)
        log_probs = F.log_softmax(logits.float(), dim=-1)
        ce = -log_probs.gather(-1, tok_d.unsqueeze(-1)).squeeze(-1)
        out[row_mask] = ce.to(dtype=out.dtype)
        return out
    ...
    def _decode_loss(
        ...
        *,
        ...
        x_pred, unembed_logits = self.net_forward(
            model_in, t, decoder_step_active=True, deterministic=False,
        )
        ce_per_token = self._ce_per_token(x_pred, tokens, unembed_logits)
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        decode_rows = decoder_step_active.bool()
        ce_per_token = self._ce_per_token(
            x_pred, tokens, unembed_logits, row_mask=decode_rows,
        )
        ...
        return self._touch_unembed_head(loss)
    ...
    def _decode_tokens(
        ...
        *,
        ...
        if not torch.isfinite(logits).all():
            raise RuntimeError("JacEllipsoid decode produced non-finite logits")
        if temperature <= 0:
    ...
    def generate(
        ...
        *,
        ...
        device = next(self.parameters()).device
        dtype = next(self.parameters()).dtype
        # ACE：未写 / false / 0 → 关闭；开启时每步从 SC 反馈减去 λ·d
        ...
        if not torch.isfinite(z).all():
            raise RuntimeError("JacEllipsoid sampling produced non-finite latents")
...
class FL_JacEllipsoidModel(FL_PreTrainedModel):
    config_class = FL_JacEllipsoidConfig
    _QDA_STATE_SUFFIXES = (
        "qda_mu",
        "qda_log_pi",
        "qda_factor",
    ...
    def __init__(self, config: FL_JacEllipsoidConfig) -> None:
        super().__init__(config)
        self.backbone = _ELFBackbone(**config.backbone_kwargs())

    def load_state_dict(self, state_dict, strict: bool = True):  # type: ignore[override]
        """``qda_table_path`` 非空时保留现场 μ/λ/logdet；learned 的 factor 仍从 ckpt 续训。"""
        sd = dict(state_dict)
        keep_local = bool(getattr(self.backbone, "qda_table_path", ""))
        if keep_local:
            learned = self.backbone.qda_mode == "learned"
            current = self.state_dict()
            for key, value in current.items():
                tail = key.rsplit(".", 1)[-1]
                if tail not in self._QDA_STATE_SUFFIXES:
                    continue
                if learned and tail == "qda_factor":
                    continue
                sd[key] = value
...
def build_model_from_config(config: FL_JacEllipsoidConfig) -> FL_JacEllipsoidModel:
    ...
    return FL_JacEllipsoidModel(config)


def build_model(cfg: dict) -> FL_JacEllipsoidModel:
    data, sampling = split_model_cfg(cfg)
    layout = token_layout_from_cfg(data)
    data.pop("tokenizer", None)
    config = FL_JacEllipsoidConfig(**data)
