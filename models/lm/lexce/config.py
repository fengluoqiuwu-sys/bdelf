"""LexCE（词形晚窗 CE）配置：ELF 双分支 + logit-normal 分位门控 CE。"""
...
class FL_LexCEConfig(PretrainedConfig):
    """LexCE：ELF 骨架 + denoise 行分位晚窗 CE（保持 logit_normal）。"""
    ...
    def backbone_kwargs(self) -> Dict[str, Any]:
        return {
            ...
            "lex_ce_mode": self.lex_ce_mode,
            "lex_ce_delta": self.lex_ce_delta,
            "lex_ce_alpha": self.lex_ce_alpha,
            "lex_ce_weight": self.lex_ce_weight,
            "lex_ce_region": self.lex_ce_region,
        }


CONFIG_CLS = FL_LexCEConfig


@dataclass
class LexCESamplingConfig:
    """推理采样（支持 SC-CFG / ACE；与训练 LexCE 门控无关）。"""
    ...
    @classmethod
    def from_dict(cls, cfg: dict) -> "LexCESamplingConfig":
