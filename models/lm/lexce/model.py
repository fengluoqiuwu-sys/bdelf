"""LexCE (Lexical late-window CE on ELF) — continuous diffusion LM with SC-CFG.
...
from models.lm.lexce.ace import apply_ace_steer, resolve_ace_steering
from models.lm.lexce.config import FL_LexCEConfig
from models.lm.lexce.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.lm.lexce.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _LexCEBackbone(nn.Module):
    """LexCE：ELF 双分支 + denoise 行分位晚窗 CE。"""
    ...
    def __init__(
        ...
        lex_ce_mode: str = "soft",
        lex_ce_delta: float = 0.15,
        lex_ce_alpha: float = 10.0,
        lex_ce_weight: float = 0.05,
        lex_ce_region: str = "late",
    ) -> None:
        super().__init__()
        ...
        self.lex_ce_mode = str(lex_ce_mode).lower()
        self.lex_ce_delta = float(lex_ce_delta)
        self.lex_ce_alpha = float(lex_ce_alpha)
        self.lex_ce_weight = float(lex_ce_weight)
        self.lex_ce_region = str(lex_ce_region).lower()
        if self.lex_ce_mode not in ("hard", "soft"):
            raise ValueError(
                f"lex_ce_mode must be 'hard' or 'soft', got {lex_ce_mode!r}"
            )
        if self.lex_ce_region not in ("late", "early"):
            raise ValueError(
                f"lex_ce_region must be 'late' or 'early', got {lex_ce_region!r}"
            )
        # 预计算分位阈值（纯 Python float），避免 torch.compile 追踪 NormalDist.inv_cdf
        self._lex_ce_t_late = self._compute_lex_ce_quantile_t(1.0 - max(self.lex_ce_delta, 0.0))
        self._lex_ce_t_early = self._compute_lex_ce_quantile_t(min(max(self.lex_ce_delta, 0.0), 1.0))
        self.last_loss_branch = ""
        self.last_l2_loss = float("nan")
        self.last_ce_loss = float("nan")
        self.last_lex_ce_loss = float("nan")
    ...
    def _compute_lex_ce_quantile_t(self, cdf_p: float) -> float:
        """训练 t 分布下的分位：P(t <= 返回值) ≈ cdf_p。

        logit_normal：t=sigmoid(N(μ,σ))；uniform：返回值=cdf_p。
        """
        p = min(max(float(cdf_p), 0.0), 1.0)
        schedule = (self.time_schedule or "logit_normal").lower()
        if schedule == "logit_normal":
            ...
            logit_t = float(self.denoiser_p_mean) + float(self.denoiser_p_std) * z
            return 1.0 / (1.0 + math.exp(-logit_t))
        return p

    def _lex_ce_threshold(self) -> float:
        """晚窗阈值 t0：使 P(t>=t0)≈delta（预计算缓存）。"""
        delta = max(float(self.lex_ce_delta), 0.0)
        if delta <= 0.0:
            return 1.0
        if delta >= 1.0:
            return 0.0
        return float(self._lex_ce_t_late)

    def _lex_ce_gate(self, t: torch.Tensor) -> torch.Tensor:
        """按 t 返回 LexCE 门控权重 (B,)；hard∈{0,1}，soft∈(0,1)。"""
        if self.lex_ce_region == "early":
            te = float(self._lex_ce_t_early)
            if self.lex_ce_mode == "hard":
                return (t <= te).to(dtype=t.dtype)
            return torch.sigmoid(self.lex_ce_alpha * (te - t))
        threshold = float(self._lex_ce_t_late)
        if self.lex_ce_mode == "hard":
            return (t >= threshold).to(dtype=t.dtype)
        return torch.sigmoid(self.lex_ce_alpha * (t - threshold))
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        # LexCE：仅 denoise 行、原始 t 落在分位晚窗时叠加轨迹 CE。
        lex_gate = self._lex_ce_gate(t)
        lex_mask = l2_mask * lex_gate.view(-1, 1)
        total_sum = (
            (ce_per_token * ce_mask).sum()
            + (l2_per_token * l2_mask).sum()
            + self.lex_ce_weight * (ce_per_token * lex_mask).sum()
        )
        ...
        self.last_lex_ce_loss = torch.where(
            lex_denom > 0,
            (ce_per_token * lex_mask).sum() / lex_denom.clamp(min=1.0),
            nan_scalar,
        ).detach()
        return loss
...
class FL_LexCEModel(FL_PreTrainedModel):
    config_class = FL_LexCEConfig

    def __init__(self, config: FL_LexCEConfig) -> None:
        super().__init__(config)
        self.backbone = _LexCEBackbone(**config.backbone_kwargs())
...
def build_model_from_config(config: FL_LexCEConfig) -> FL_LexCEModel:
    ...
    return FL_LexCEModel(config)


def build_model(cfg: dict) -> FL_LexCEModel:
    data, sampling = split_model_cfg(cfg)
    layout = token_layout_from_cfg(data)
    data.pop("tokenizer", None)
    config = FL_LexCEConfig(**data)
