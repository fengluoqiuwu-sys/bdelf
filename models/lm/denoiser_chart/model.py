"""DenoiserChart：ELF 复制体 + bottleneck 隔离 W_t + 中段冻 unembed CE。

骨架、路径、双分支混损与 SC-CFG 均同 ELF。相对 ELF 只多：
- denoise 在共享 B 之后插入可逆对角仿射 W_t（decode 默认不套）；
- denoise 中段对 x_hat_t 加冻 unembed token CE（L_chart）。
"""
...
from models.lm.denoiser_chart.ace import (
    ace_step_active,
    apply_ace_steer,
    parse_ace_step_range,
    resolve_ace_steering,
)
from models.lm.denoiser_chart.config import FL_DenoiserChartConfig
from models.lm.denoiser_chart.layers import (
    BottleneckTextProj,
    DenoiserChartWarp,
    ELFBlock,
    FinalLayer,
...
from models.lm.denoiser_chart.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _DenoiserChartBackbone(nn.Module):
    """ELF 双分支骨架 + 仅 denoise 可见的 W_t + 中段 L_chart。"""
    ...
    def __init__(
        ...
        chart_warp: bool = True,
        chart_weight: float = 0.1,
        chart_t_min: float = 0.3,
        chart_t_max: float = 0.8,
        chart_leak_decode: bool = False,
        ...
        self.chart_warp_enabled = bool(chart_warp)
        self.chart_weight = float(chart_weight)
        self.chart_t_min = float(chart_t_min)
        self.chart_t_max = float(chart_t_max)
        self.chart_leak_decode = bool(chart_leak_decode)
        self.freeze_unembed = bool(freeze_unembed)
        self.last_loss_branch = ""
        self.last_l2_loss = float("nan")
        self.last_ce_loss = float("nan")
        self.last_chart_ce_loss = float("nan")
        ...
        self.chart_affine = DenoiserChartWarp(bottleneck_dim)
        if self.freeze_unembed:
            self.unembed_kernel.requires_grad_(False)
            self.unembed_bias.requires_grad_(False)
    ...
    def _apply_chart_warp(
        self,
        h: torch.Tensor,
        t: torch.Tensor,
        decoder_step_active: bool | torch.Tensor | None,
    ) -> torch.Tensor:
        """仅 denoise 行套 W_t；decode 默认恒等。泄漏对照则两模式都套。"""
        if not self.chart_warp_enabled:
            return h
        h_w = self.chart_affine(h, t)
        if self.chart_leak_decode:
            return h_w
        if decoder_step_active is None:
            return h_w
        if isinstance(decoder_step_active, torch.Tensor) and decoder_step_active.dim() > 0:
            gate = decoder_step_active.to(dtype=h.dtype).view(-1, 1, 1)
            return gate * h + (1.0 - gate) * h_w
        if float(decoder_step_active):
            return h
        return h_w
    ...
    def net_forward(
        ...
        *,
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            t_f = t.to(dtype=param_dtype)
            h = self.text_proj.proj1(x_f)
            h = self._apply_chart_warp(h, t_f, decoder_step_active)
            x_h = self.text_proj.proj2(h)
            sc_cfg_scale_emb = (
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        chart_sum = ce_per_token.new_zeros(())
        chart_denom = ce_per_token.new_zeros(())
        if self.chart_weight > 0:
            logits_u = (
                x_pred.to(dtype=self.unembed_kernel.dtype) @ self.unembed_kernel.detach()
                + self.unembed_bias.detach()
            )
            chart_log_probs = F.log_softmax(logits_u.float(), dim=-1)
            chart_ce_per_token = -chart_log_probs.gather(
                -1, tokens.unsqueeze(-1),
            ).squeeze(-1)
            t_mid = (
                (t >= self.chart_t_min) & (t <= self.chart_t_max)
            ).to(dtype=ce_per_token.dtype).view(-1, 1)
            chart_mask = l2_mask * t_mid
            chart_sum = self.chart_weight * (chart_ce_per_token * chart_mask).sum()
            chart_denom = chart_mask.sum()
            self.last_chart_ce_loss = torch.where(
                chart_denom > 0,
                (chart_ce_per_token * chart_mask).sum() / chart_denom.clamp(min=1.0),
                torch.full(
                    (), float("nan"),
            ...
            self.last_chart_ce_loss = torch.full(
                (), float("nan"),
                device=ce_per_token.device, dtype=ce_per_token.dtype,
            ).detach()
        total_sum = (
        ...
        # warp 在 chart_warp=false 消融时仍需进图，避免 DDP unused param。
        loss = loss + 0.0 * (
            self.chart_affine.to_ab.weight.sum() + self.chart_affine.to_ab.bias.sum()
        )
        return loss
    ...
    def _touch_unused_heads(self, loss: torch.Tensor) -> torch.Tensor:
        ...
        touch = touch + self.chart_affine.to_ab.weight.sum() + self.chart_affine.to_ab.bias.sum()
...
class FL_DenoiserChartModel(FL_PreTrainedModel):
    config_class = FL_DenoiserChartConfig

    def __init__(self, config: FL_DenoiserChartConfig) -> None:
        super().__init__(config)
        self.backbone = _DenoiserChartBackbone(**config.backbone_kwargs())
...
def build_model_from_config(config: FL_DenoiserChartConfig) -> FL_DenoiserChartModel:
    ...
    return FL_DenoiserChartModel(config)


def build_model(cfg: dict) -> FL_DenoiserChartModel:
    data, sampling = split_model_cfg(cfg)
    layout = token_layout_from_cfg(data)
    data.pop("tokenizer", None)
    config = FL_DenoiserChartConfig(**data)
