...
class FL_LoopSCConfig(PretrainedConfig):
    """LoopSC 配置：骨干同 ELF，训练期把采样 SC 环展开 K 步。"""

    model_type = "fl_loopsc"
    ...
    def backbone_kwargs(self) -> Dict[str, Any]:
        return {
            ...
            "sc_unroll_steps": self.sc_unroll_steps,
            "sc_unroll_bptt": self.sc_unroll_bptt,
            "sc_unroll_detach_z": self.sc_unroll_detach_z,
        }


CONFIG_CLS = FL_LoopSCConfig


@dataclass
class LoopSCSamplingConfig:
    """LoopSC 推理配置（SC-CFG / ACE 与 ELF 同构；默认 ACE=off）。"""
    ...
    @classmethod
    def from_dict(cls, cfg: dict) -> "LoopSCSamplingConfig":
