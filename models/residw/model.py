"""ResidW — ELF 复制体：SC-CFG 老师只蒸馏位置残差。

骨干与推理同 ELF（arXiv:2605.10938）。训练期 SC-CFG 把
...
from models.residw.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.residw.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _ResidWBackbone(nn.Module):
    """ResidW DiT 骨干：denoise MSE / decode CE，SC-CFG 老师为残差高通。"""
    ...
    def _mixed_branch_loss(
        ...
        *,
        ...
        """ELF 式 denoise/decode 混合；SC-CFG 老师只含 HP(v_c-v_∅)。"""
        bsz, seq_len, _ = x0.shape
        device = x0.device
        dtype = x0.dtype
        ...
        if self.num_self_cond_cfg_tokens > 0:
            ...
            delta_v = self._highpass_seq(v_cond - v_uncond, loss_mask)
            sc_w = self_cond_cfg.reshape(-1, 1, 1)
            sc_guidance = (1.0 - 1.0 / sc_w) * delta_v
