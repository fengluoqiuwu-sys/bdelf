"""ResidW 配置（ELF 复制体；SC-CFG 老师为残差增益）。"""
...
class FL_ResidWConfig(PretrainedConfig):
    """ResidW 配置：骨干同 ELF，训练期 SC-CFG 只拟合位置残差。"""

    model_type = "fl_residw"
...
CONFIG_CLS = FL_ResidWConfig


@dataclass
class ResidWSamplingConfig:
    """ResidW 推理配置（SC-CFG / ACE 与 ELF 同构）。"""
    ...
    @classmethod
    def from_dict(cls, cfg: dict) -> "ResidWSamplingConfig":
