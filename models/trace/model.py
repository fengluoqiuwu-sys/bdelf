...
长训由 ``models.trace.track`` 低频用 EMA 权重跑 ACE Alg.1 再 EMA 更新 ``d``。
推理默认 ACE=off，不再减 ``λd``。
...
from models.trace.ace import apply_ace_steer, resolve_ace_steering
from models.trace.config import FL_TrACEConfig
from models.trace.layers import (
    BottleneckTextProj,
    ELFBlock,
    FinalLayer,
    TextRotaryEmbedding,
...
from models.trace.t5_encoder import (
    T5Encoder,
    encode_text,
...
class _TrACEBackbone(nn.Module):
    ...
    def _load_attr_d_init(self, attr_d_init: str | None) -> None:
        ...
        if raw.lower() == "random":
            ...
            return
        from models.trace.ace import load_ace_direction
