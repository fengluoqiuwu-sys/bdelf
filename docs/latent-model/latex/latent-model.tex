...
T5 \textbf{不}扫瓶颈：$B$ 一律 $64$。合计 \textbf{13} 组：VAE 8 + T5 5。
...
固定 $B{=}64$（配置默认，不扫 $16/32/128$）。消融只做方向、readout，以及单向时的 $D$。
...
\midrule
双向 & \texttt{e} & 64 & --- & 1 & 1 \\
单向 & \texttt{e} & 64 & 1, 16 & 2 & 2 \\
单向 & \texttt{b} & 64 & 1, 16 & 2 & 2 \\
\midrule
 &  &  &  &  & \textbf{5} \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  \item \textbf{双向 / readout=\texttt{e}}：默认 \texttt{bidirectional=true}（无 $D$）。
  \item \textbf{单向 / readout=\texttt{e}}：$D{=}1$、$D{=}16$。
  \item \textbf{单向 / readout=\texttt{b}}：$D{=}1$、$D{=}16$。
\end{itemize}

CLI 对应：双向不改默认；单向 \texttt{--set model.bidirectional=false}；
\texttt{readout=b} 时 \texttt{--set model.readout=b}；$D$ 为 \texttt{model.block\_size}。$B$ 用配置默认 $64$，不必 \texttt{--set model.latent\_dim}。
