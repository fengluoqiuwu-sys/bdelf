...
$B$ 为 \texttt{latent\_dim}；$D$ 为 \texttt{block\_size}。本矩阵\textbf{只跑} $D{=}1$（逐 token 因果），不扫块因果 $D{=}16$。
T5 的 readout 为 \texttt{e}（$z\in\mathbb{R}^{E}$）或 \texttt{b}（$z\in\mathbb{R}^{B}$）；双向 T5 无 $D$。
T5 \textbf{不}扫瓶颈：$B$ 一律 $64$。合计 \textbf{7} 组：VAE 4 + T5 3。
...
VAE 一个 4 卡作业内按 $B$ 升序串跑四组（均为 $D{=}1$）。
T5 在 \texttt{train-server-2} 上按下列格点训练。

\subsection{\texttt{latent\_vae}（ovan-server，4\texttimes4090）}

固定：$B$ 读出、逐 token 因果 encoder、并行 decoder，$D{=}1$。只扫 $B{\in}\{16,32,64,128\}$，共 4 组。
...
固定 $B{=}64$（配置默认，不扫 $16/32/128$）、单向 $D{=}1$。消融只做方向与 readout。
...
\begin{itemize}
  ...
  \item \textbf{单向 / readout=\texttt{e}}：$D{=}1$。
  \item \textbf{单向 / readout=\texttt{b}}：$D{=}1$。
\end{itemize}

CLI 对应：双向不改默认；单向 \texttt{--set model.bidirectional=false}；
\texttt{readout=b} 时 \texttt{--set model.readout=b}。$B$ 用配置默认 $64$，不必 \texttt{--set model.latent\_dim}；单向 $D$ 用默认 \texttt{block\_size=1}。
