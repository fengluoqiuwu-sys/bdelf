...
$B$ 为 \texttt{latent\_dim}；$D$ 为 \texttt{block\_size}（$D{=}1$ 逐 token 因果，$D{=}16$ 块因果）；T5 的 readout 为 \texttt{e}（$z\in\mathbb{R}^{E}$）或 \texttt{b}（$z\in\mathbb{R}^{B}$）。
双向 T5 无 $D$（encoder 与 decoder self-attn 全双向，不走块 mask）。
合计 \textbf{20} 组：VAE 8 + T5 12。
...
模型 & 服务 & 调度 & 可见 GPU & 单卡 \\
\midrule
\texttt{latent\_vae} & \texttt{ovan-server} & Slurm（\texttt{cls1}，QOS \texttt{long}） & 4$\times$ RTX 4090 & 24\,GiB \\
\texttt{latent\_t5} & \texttt{train-server-2} & common & 8$\times$ RTX 5090 & 32\,GiB \\
\bottomrule
\end{tabular}
\end{center}

VAE 拆成两个 4 卡作业：作业内按 $B$ 升序串跑；作业 1 为 $D{=}1$ 四组，作业 2 为 $D{=}16$ 四组（依赖作业 1 结束后再占卡）。
T5 在 \texttt{train-server-2} 上按下列格点训练。

\subsection{\texttt{latent\_vae}（ovan-server，4\texttimes4090）}

固定：$B$ 读出、因果/块因果 encoder、并行 decoder。扫 $B{\in}\{16,32,64,128\}$ 与 $D{\in}\{1,16\}$，共 8 组。

\begin{center}
\small
\begin{tabular}{lcc}
\toprule
$B \backslash D$ & $D{=}1$（逐 token 因果） & $D{=}16$（块因果） \\
\midrule
16 & $\checkmark$ & $\checkmark$ \\
32 & $\checkmark$ & $\checkmark$ \\
64 & $\checkmark$ & $\checkmark$ \\
128 & $\checkmark$ & $\checkmark$ \\
\bottomrule
\end{tabular}
\end{center}

\subsection{\texttt{latent\_t5}（train-server-2，8\texttimes5090）}
...
\midrule
双向 & \texttt{e} & 32, 64 & --- & 2 & 2 \\
单向 & \texttt{e} & 16, 32, 64, 128 & 1, 16 & $4{\times}2$ & 8 \\
单向 & \texttt{b} & 32, 64 & 1 & 2 & 2 \\
\midrule
 &  &  &  &  & \textbf{12} \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  \item \textbf{双向 / readout=\texttt{e}}：$(B{=}32)$、$(B{=}64)$。
  \item \textbf{单向 / readout=\texttt{e}}：$B{\in}\{16,32,64,128\}$、$D{\in}\{1,16\}$ 全组合（8 组）：
    \begin{itemize}
      \item $D{=}1$：$(16,1)$、$(32,1)$、$(64,1)$、$(128,1)$
      \item $D{=}16$：$(16,16)$、$(32,16)$、$(64,16)$、$(128,16)$
    \end{itemize}
  \item \textbf{单向 / readout=\texttt{b}}：$(B{=}32,D{=}1)$、$(B{=}64,D{=}1)$。
\end{itemize}

CLI 对应：双向默认 \texttt{bidirectional=true}；单向 \texttt{--set model.bidirectional=false}；
\texttt{readout=b} 时 \texttt{--set model.readout=b}；$B,D$ 为 \texttt{model.latent\_dim}、\texttt{model.block\_size}。
...
当前 Stage-1 训练矩阵（VAE：ovan-server 4\texttimes4090；T5：train-server-2 8\texttimes5090）见第~\ref{sec:runs} 节。
