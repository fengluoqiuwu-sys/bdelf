...
\subsection{\texttt{latent\_t5}：可选双向/单向 encoder、AR decoder}

\paragraph{潜在优势}
\begin{itemize}
  \item \textbf{双向 $z_t$（默认 \texttt{bidirectional=true}）}：encoder 见全句，$z_t$ 可编码左右语境；对指代消解、长程依赖更友好，作为 DLM「语义坐标」更贴近 BERT/T5 encoder 表征~\cite{devlin2019bert,raffel2020t5}。
  \item \textbf{单向 encoder 消融}：\texttt{bidirectional=false} 时与 \texttt{latent\_vae} 同为逐 token 因果视野，可在\textbf{同一 AR decoder 拓扑}下隔离「encoder 双向性」对 $z$ 的影响。
  \item \textbf{AR 重建}：$p(x|z)$ 为自回归，与主流 LM 评测与课程学习兼容；前缀条件时 decoder 自然续写，便于「前缀 $\to$ encode $z$ $\to$ AR 补全」推理。
  \item \textbf{span 辅助}：腐蚀 encoder 输入、decoder 仍重建原句，类似去噪自编码，可能使 $z$ 对局部缺失更鲁棒，利于 Stage-2 对 $z$ 的随机扰动。
  \item \textbf{\texttt{readout=b}}：与 VAE 共享 $z\in\mathbb{R}^B$，便于在\textbf{同一扩散维度}下比较「双向+AR」与「因果+并行」谁学得更好。
\end{itemize}
...
\texttt{max\_seq\_len} & \multicolumn{2}{c}{1024} \\
\texttt{tokenizer} & \multicolumn{2}{c}{GPT-2（OWT 管线）} \\
encoder 注意力 & \texttt{bidirectional}（默认 true；可 false） & 禁止双向；\texttt{block\_size}（默认 1） \\
...
\begin{enumerate}
  \item \textbf{双向}（\texttt{bidirectional=true}，\texttt{latent\_t5} 默认）：self-attention 无因果 mask，等价于 T5 encoder 的「全上下文」语义（算子仍为 RoPE+GELU）。\textbf{仅} \texttt{latent\_t5} 可配置。
  \item \textbf{逐 token 因果}（\texttt{bidirectional=false}, \texttt{block\_size=1}）：\texttt{scaled\_dot\_product\_attention(..., is\_causal=True)}；\textbf{不}构造块 mask，避免 $B{=}1$ 时不必要的开销。\texttt{latent\_t5} 可通过 \texttt{--set model.bidirectional=false} 选用；\texttt{latent\_vae} 固定此模式（默认 \texttt{block\_size=1}）。
...
\subsection{\texttt{latent\_t5}：AR + cross-attn}

每层：因果 self-attn（RoPE）$\to$ cross-attn($h_{\mathrm{dec}}, z$) $\to$ FFN。

\paragraph{训练移位}
数据管线因 \texttt{full\_sequence\_training=True} 传入全长 $x$。
模型内部构造 decoder 输入 $\tilde{x} = [\mathrm{BOS}, x_1,\ldots,x_{L-1}]$，encoder 仍看完整 $x$。
\textbf{不}使用 GPT 式外层 \texttt{batch[:,:-1]} 移位（否则 encoder 会丢失末 token）。

\paragraph{生成}
\begin{itemize}
  \item 有前缀：将前缀 pad 至 $L$，encode 得 $z$；自前缀末 token 起 AR 续写。
  \item 无条件：从 $\mathcal{N}(0,I)$ 采样 $z$，形状为 $(L,E)$ 或 $(L,B)$（随 \texttt{readout}）；再 AR。宽先验（$E{=}512$）下无条件样本质量可能较差，属已知限制。
\end{itemize}

\subsection{\texttt{latent\_vae}：统一生成}

Decoder 仅接收 $\mathrm{from\_latent}(z)$，与 Cola \texttt{decode\_logits} 同型。
生成：encode 后一次 decode，或从先验采 $z\in\mathbb{R}^{L\times B}$ 再 decode。
...
\begin{description}
  \item[\texttt{latent\_t5}] 教师强制自回归交叉熵：对全长 $x$，$\mathcal{L}_{\mathrm{recon}} = -\sum_t \log p_\theta(x_t \mid x_{<t}, z)$。
  \item[\texttt{latent\_vae}] 并行 CE：$-\sum_t \log p_\theta(x_t \mid z)$，所有位置同时预测。
\end{description}

Held-out 评测：T5 用 teacher-forced AR CE；VAE 用并行重建 CE（\texttt{config/train/eval/latent/default.yaml}）。
...
\begin{itemize}
  ...
  \item 阶段内计算图钉死为该阶段最大 $L$；更短桶样本右 pad 到 $L$，loss 对 pad mask。
  \item 同一步只含同一桶，避免步间改 $L$ 或全局条数。
\end{itemize}

\subsection{四阶段与配比}

每步约 $262\text{K}$ pad-token（$=$ 图 $L \times$ 全局条数；方案 2：$L$ 加倍则条数减半）。
...
\texttt{latent\_t5} 与 \texttt{latent\_vae} 在共享 T5-small 维数 encoder 与 Cola 式 ELBO 下，分别检验「（双向或单向）encoder + AR + span 辅助」与「因果 + 并行解码 + BERT-mask」两条 Stage-1 路线（动机与 DLM 语义空间讨论见第~\ref{sec:why}--\ref{sec:dlm-space} 节）。
