...
本文档亦说明\textbf{为何}在 Cola 路线之外做这组对照，二者作为扩散语言模型（DLM）Stage-2 \emph{语义空间}的初步优劣分析，以及当前 Stage-1 长度课程的训练矩阵（第~\ref{sec:runs} 节）。
...
\begin{itemize}
  \item \textbf{非} HuggingFace 冻结 \texttt{t5-small}（ELF 家族用法）。\texttt{readout=e|b} 为手写 RoPE Transformer；\texttt{readout=none} 算子对齐原版 \texttt{t5-small}（相对位置偏置、RMSNorm、ReLU FFN），但 decoder 写死双向、词表仍为 GPT-2。
  \item \textbf{非} \texttt{cola\_vae} 替换品；Cola Stage-2 仍只加载 \texttt{cola\_vae}。
  \item 二者注册为 \texttt{kind=latent}，训练日志字段与 \texttt{cola\_vae} 同构（\texttt{recon\_ce}、\texttt{kl}、\texttt{mask} 等）。
\end{itemize}
...
默认 $B{=}64$ 时 \texttt{latent\_vae} 的 KL 在 $B$ 维；
\texttt{latent\_t5}（\texttt{readout=b}）默认 $B{=}32$，KL 同在瓶颈维；
\texttt{readout=e} 则在 $E{=}512$ 维施加 KL，先验负担显著更重。
\texttt{readout=none} 无高斯后验、无 KL。
...
\midrule
$z$ 默认维数 & $B{=}64$ & $E{=}512$ 或 $B{=}32$（\texttt{readout}）；\texttt{none} 无 $z$ \\
...
\texttt{latent\_dim}（$B$） & 32 & 64 \\
\texttt{max\_seq\_len} & \multicolumn{2}{c}{4096} \\
\texttt{tokenizer} & \multicolumn{2}{c}{GPT-2（OWT 管线）} \\
encoder / decoder self-attn & encoder：\texttt{bidirectional}；decoder：\texttt{decoder\_bidirectional}（null=同 encoder） & 禁止双向；\texttt{block\_size}（默认 1） \\
读出 & \texttt{readout=e|b|none} & 固定 $B$ 读出 \\
辅助损失 & \texttt{lambda\_span} + span 腐蚀 & \texttt{lambda\_mask} + BERT-mask \\
\texttt{beta\_kl} & \multicolumn{2}{c}{0.1} \\
\texttt{kl\_entropy} & \multicolumn{2}{c}{缺键/\texttt{false} 同旧哈希（默认 YAML 关）；开：$\mathrm{KL}+\mathbb{E}[\log q]$} \\
\bottomrule
\end{tabular}
\end{center}

与 T5-small 的 $d_{\mathrm{model}}, d_{\mathrm{kv}}, d_{\mathrm{ff}}, 6{+}6$ 层对齐。
\texttt{readout=e|b} \textbf{未}复现 T5 相对位置偏置、RMSNorm 与 ReLU FFN，而采用本仓库 AR 栈惯例：pre-norm LayerNorm、RoPE~\cite{su2024rope}、GELU FFN。
\texttt{readout=none}（原 T5）块算子对齐 HuggingFace 原版 \texttt{t5-small}：相对位置偏置（32 buckets）、T5LayerNorm（无均值、无 bias 的 RMSNorm，$\varepsilon{=}10^{-6}$）、DenseReluDense（ReLU、无 gate）、独立 $Q,K,V,O$ 投影（注意力分数不除 $\sqrt{d_{\mathrm{kv}}}$）、基础词表 embedding 与 \texttt{lm\_head} 权重绑定。
用户写死双向：encoder 与 decoder self-attn 均为全双向，decoder 的 relative bias 亦按双向桶（与 encoder 同构），\textbf{不}使用原版 T5 的 causal decoder。

\subsection{共享模块 \texttt{models/latent/encdec/}}

\begin{description}
  \item[\texttt{SelfAttention}]（\texttt{encdec}，\texttt{readout=e|b}）$Q,K,V$ 投影至 $H \cdot d_{\mathrm{kv}}$；RoPE 施加于 $Q,K$。顺序位置 $0{\ldots}L{-}1$ 切预计算表（默认长度 4096），超长回退按位置现场计算。
  \item[\texttt{TransformerBlock}] pre-norm self-attn + FFN（精确 GELU，非 tanh 近似）。dropout $p{=}0$ 时为 Identity。
...
\subsection{原 T5 块（\texttt{models/latent/latent\_t5/t5\_blocks.py}）}

仅 \texttt{readout=none} 组装。\texttt{T5LayerNorm}、相对位置偏置（首层共享）、\texttt{T5DenseReluDense}、独立 $Q/K/V/O$。Decoder 每层为 self-attn $\to$ cross-attn $\to$ FFN。
相对位置桶按 $(Q,K,\mathrm{device})$ 缓存（与权重无关）；无相对偏置的层不物化全零 $L{\times}L$ bias，以便 SDPA 走 Flash。
...
\begin{enumerate}
  \item \textbf{双向}（\texttt{bidirectional=true}，\texttt{latent\_t5} 默认）：encoder 无因果 mask。Decoder 默认同为双向，从 $z$ 并行重建（不 teacher-force $x$）。\textbf{仅} \texttt{latent\_t5} 可配置。
  \item \textbf{逐 token 因果}（\texttt{bidirectional=false}, \texttt{block\_size=1}）：encoder 为 \texttt{is\_causal=True}；decoder 默认同为因果 AR + cross-attn。\texttt{latent\_t5} 可通过 \texttt{--set model.bidirectional=false} 选用；\texttt{latent\_vae} 固定此模式（默认 \texttt{block\_size=1}）。
...
\texttt{latent\_t5} 另有 \texttt{decoder\_bidirectional}：\texttt{null} 时 decoder 跟随 encoder；显式 \texttt{false} 时 decoder 因果 AR（仅 \texttt{readout=e|b}）。
\texttt{readout=none}（原 T5）\textbf{写死} encoder 与 decoder 均为双向，禁止单向。
\texttt{latent\_vae} \textbf{禁止} \texttt{bidirectional=true}（配置层与实现层均拒绝；encoder 写死 \texttt{False}）。

Pad（独立 special，非 EOS）不进 CE / 后验平均；BERT-mask 与 span 不抽 pad、也不抽第 0 位。
双向 self-attn 与 T5 cross-attn 屏蔽 pad key（\texttt{latent\_t5} 始终传入 bool pad；全 False 与 \texttt{None} 对注意力分数等价）。
逐 token 因果 self-attn \textbf{不加} pad mask（右 pad 下真实 token 看不到右侧 pad，以便 Flash）；\texttt{block\_size>1} 时块内双向叠加 pad mask。
...
\texttt{latent\_t5} 与 \texttt{latent\_vae} 另有开关 \texttt{kl\_entropy}（不进必填键；缺键或 \texttt{false} 不进指纹，与旧 YAML 同哈希）。默认 YAML 关：后验项即为上式 KL。开时两项并存、同一 $\beta$：
\begin{equation}
  \mathcal{R}(q)=\mathrm{KL}(q\|p_0)+\mathbb{E}[\log q],\qquad
  \mathbb{E}[\log q] = -\tfrac{1}{2}\,\mathbb{E}\bigl[1 + \log 2\pi + \log\sigma^2\bigr].
\end{equation}
\(\mathbb{E}[\log q]\) 是对角高斯熵的相反数（不含 $\mu$），最小化则抬高 $\sigma$；prior-KL 仍约束 $\mu$。开时 artifact tag 后缀 \texttt{-sigma}。\texttt{readout=none} 无后验，$\mathcal{R}=0$。
...
\subsubsection{\texttt{readout=none}：无瓶颈}

不建 \texttt{Posterior*Readout}，decoder memory 即为 encoder 隐状态 $h\in\mathbb{R}^{L\times E}$。
无重参数化、KL 记 0。encoder 与 decoder \textbf{写死双向}（并行从 $h$ 重建）；\texttt{bidirectional=false} 或 \texttt{decoder\_bidirectional=false} 会报错。
块算子走 \texttt{models/latent/latent\_t5/t5\_blocks.py}（原版 t5-small），\textbf{不}走 \texttt{encdec} 的 RoPE 栈。

\section{Decoder 与生成}

\subsection{\texttt{latent\_t5}：cross-attn decoder（self-attn 与 encoder 同模式）}

每层：self-attn $\to$ cross-attn($h_{\mathrm{dec}}, z$) $\to$ FFN。
\texttt{readout=e|b} 的 self-attn 为 RoPE（默认与 encoder 的 \texttt{bidirectional} 一致，可被 \texttt{decoder\_bidirectional} 覆盖）；
\texttt{readout=none} 的 self-attn 为 T5 相对位置偏置（双向桶）且写死双向。

\paragraph{训练}
\begin{itemize}
  \item decoder 双向：decoder 从 $z$ 起步（\texttt{readout=e}/\texttt{none} 时已是 $E$ 维；\texttt{readout=b} 时经 $B\to E$），一次预测全长 $x$。数据仍因 \texttt{full\_sequence\_training=True} 传入全长。
  \item decoder 因果：构造 $\tilde{x} = [\mathrm{BOS}, x_1,\ldots,x_{L-1}]$ 做教师强制 AR。\textbf{不}使用 GPT 式外层 \texttt{batch[:,:-1]} 移位（否则 encoder 会丢失末 token）。
\end{itemize}

\paragraph{生成}
\begin{itemize}
  \item decoder 双向：一次 decode 全长；有前缀则 encode（前缀+$L$ pad）后并行重建并覆盖前缀。无条件：VAE 读出从先验采 $z$；\texttt{readout=none} 则 encode 首位 BOS + 其余 pad。
  \item decoder 因果（仅 \texttt{e|b}）：有前缀则 encode 得 memory 再 AR 续写；无条件从 $\mathcal{N}(0,I)$ 采 $z$ 再 AR。
\end{itemize}
...
\texttt{kl\_entropy} 为假（缺键 / \texttt{false}，不进指纹；默认 YAML）时 $\mathcal{R}=\mathrm{KL}(q\| \mathcal{N}(0,I))$；为真时
\begin{equation}
  \mathcal{R}=\mathrm{KL}(q\| \mathcal{N}(0,I))+\mathbb{E}[\log q].
\end{equation}
\texttt{readout=none} 无后验，$\mathcal{R}=0$。\texttt{cola\_vae} 无此开关，仍只用 prior-KL。

\subsection{重建项 $\mathcal{L}_{\mathrm{recon}}$}

\begin{description}
  \item[\texttt{latent\_t5}] decoder 双向（默认）：并行 CE $-\sum_t \log p_\theta(x_t \mid z)$。decoder 因果：教师强制 AR CE $-\sum_t \log p_\theta(x_t \mid x_{<t}, z)$。\texttt{readout=none} 时 $z$ 即为 $h$。
...
encode 被 mask 的序列，仅在 mask 位置对原始 $x$ 计 CE~\cite{devlin2019bert}。
固定张量形状（\texttt{ignore\_index} 掩掉非 mask 位），便于 \texttt{torch.compile}。
无 mask 位时辅助项为 $0$（不是 NaN）。

\paragraph{\texttt{latent\_t5}：原地 span CE}
受 T5 span corruption 启发~\cite{raffel2020t5}，但\textbf{不}采用变长 decoder 目标序列（与定长训练图不兼容）。
流程：
\begin{enumerate}
  \item 按 span 级 mask 覆盖约 15\% token（平均 span 长 3；允许重叠；不抽第 0 位）；涂色在 CPU 上完成再拷到 GPU；
  \item encoder 输入将 span 换为 sentinel id $V{+}k$，$k\in\{0,\ldots,99\}$；
  \item decoder 仍重建全长原始 $x$（因果时 teacher-force 为原序列右移，aux 半段重复同一输入）；
  \item CE 仅在腐蚀位置计算；无 span 位时辅助项为 $0$。
\end{enumerate}
100 个 sentinel 仅扩展 encoder embedding，\textbf{不}进入 \texttt{lm\_head}。

\paragraph{训练图：清洁 / aux 沿 batch 拼接}
训练态若 $\lambda{>}0$，将清洁序列与腐蚀 / BERT-mask 序列沿 batch 维拼接，一次 encode+decode。
self-attn 不跨样本，与两次独立前向同分布。
重建 CE 与后验 $\mathcal{R}$ 只用清洁半段；aux CE 只用 mask / span 位。
评测 \texttt{forward} 不开 aux。
...
\subsection{训练循环（与课程存盘）}

\texttt{train/loop.py} 在 checkpoint 写完\textbf{之后}才预取下一步 CPU batch，以免 \texttt{curriculum\_state} 的 bucket 被写成下一步。
非有限 loss 且权重仍有限时跳过 backward（避免 DDP 中毒）；梯度非有限则跳过 optimizer step。
...
\section{当前 Stage-1 训练矩阵}
\label{sec:runs}

本节记录\textbf{正在进行 / 即将按此清单执行}的长度课程训练（协议同第~\ref{sec:curriculum} 节：10B 有效 token、\texttt{100m-curriculum}、\texttt{preprocess=latent-curriculum}、OWT）。
超参以 CLI \texttt{--set} 覆盖，进入配置哈希；每组独立 run 目录。
$B$ 为 \texttt{latent\_dim}；$D$ 为 \texttt{block\_size}。
VAE 在 $D{=}1$（逐 token 因果）上扫全部 $B$；另在 $B{=}32$ 上扫块因果 $D{\in}\{16,32\}$（相对原 $D{=}1$ 四组\textbf{增加两组}）。
T5 的 readout 为 \texttt{e}（$z\in\mathbb{R}^{E}$）、\texttt{b}（$z\in\mathbb{R}^{B}$）或 \texttt{none}（无瓶颈）；双向 T5 无 $D$。
T5 有瓶颈的三组 $B$ 一律 $32$（配置默认，不扫 $16/64/128$）。另增一组原 T5（\texttt{readout=none}）。合计 \textbf{10} 组：VAE 6 + T5 4。
...
模型 & 服务 & 调度 & 可见 GPU & 单卡 \\
\midrule
\texttt{latent\_vae}、\texttt{latent\_t5} & \texttt{ovan-server} & Slurm（\texttt{cls1}，QOS \texttt{long}） & 4$\times$ RTX 4090 & 24\,GiB \\
\bottomrule
\end{tabular}
\end{center}

两条模型共用 ovan-server 4 卡、QOS \texttt{long}。
VAE 两个 4 卡作业：按 $B$ 升序串跑 $D{=}1$ 四组；对 $B{=}32$ 串跑 $D{=}16$ 再 $D{=}32$。
T5 两个 4 卡作业：单向 \texttt{e} 再 \texttt{b} 串跑；双向默认 \texttt{e} 再原版 \texttt{none} 串跑。

\subsection{\texttt{latent\_vae}（ovan-server，4\texttimes4090）}

固定：$B$ 读出、并行 decoder。主扫 $D{=}1$（因果 encoder）的 $B{\in}\{16,32,64,128\}$；
$B{=}32$ 再扫块因果 $D{\in}\{16,32\}$。共 6 组；提交为两个 Slurm（$D{=}1$ 四段、$B{=}32$ 两段）。
...
$B$ & 16 & 32 & 64 & 128 \\
\midrule
$D{=}1$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
$D{=}16$ &  & $\checkmark$ &  &  \\
$D{=}32$ &  & $\checkmark$ &  &  \\
\bottomrule
\end{tabular}
...
\subsection{\texttt{latent\_t5}（ovan-server，4\texttimes4090）}

有瓶颈三组固定 $B{=}32$（配置默认）、单向 $D{=}1$。另增原 T5 一组（无瓶颈）。
提交：单向两组一个 Slurm；双向与原版一个 Slurm。
...
\begin{itemize}
  \item \textbf{双向 / readout=\texttt{e}}：默认 \texttt{bidirectional=true}，$B{=}32$（无 $D$）。
  \item \textbf{单向 / readout=\texttt{e}}：$D{=}1$，$B{=}32$。
  \item \textbf{单向 / readout=\texttt{b}}：$D{=}1$，$B{=}32$。
  \item \textbf{原 T5}：\texttt{readout=none}；encoder/decoder \textbf{写死双向}。无读出层、无 KL。
\end{itemize}

CLI 对应：双向不改默认；单向 \texttt{--set model.bidirectional=false}；
\texttt{readout=b} 时 \texttt{--set model.readout=b}。
有瓶颈组 $B$ 用配置默认 $32$，不必 \texttt{--set model.latent\_dim}；单向 $D$ 用默认 \texttt{block\_size=1}。
原 T5：\texttt{--set model.readout=none}（不必也不许改方向）。

\section{设计选择与局限}

\begin{enumerate}
  \item \textbf{RoPE 而非 T5 相对偏置}（仅 \texttt{readout=e|b}）：与仓库 AR/块因果路径统一。\texttt{readout=none} 已改用原版 t5-small 相对偏置 + RMSNorm + ReLU FFN；decoder 仍写死双向（相对 bias 亦为双向桶）。
  \item \textbf{GPT-2 词表}：与 OWT 预处理一致，不换 SentencePiece 32k。\texttt{readout=none} 将 \texttt{lm\_head} 与基础 vocab embed tie（sentinel 行不进 \texttt{lm\_head}）；\texttt{e|b} 仍不 tie。
  \item \textbf{KL 空间}：\texttt{readout=e} 时 KL 在 $E{=}512$；与 \texttt{latent\_vae} 默认 $B{=}64$ 后验容量不对等；\texttt{readout=b}（$B{=}32$）可缓解；\texttt{none} 无 KL。
  \item \textbf{span 辅助损失}：固定长度「原地 CE」是工程折中，非严格 T5 预训练目标。原 T5 消融只去掉瓶颈并写死双向，损失仍为重建+span 辅助。
  \item \textbf{无条件生成}：VAE 读出依赖宽先验 $z$；\texttt{readout=none} 无条件 encode 首位 BOS + 其余 pad。实用场景建议前缀 encode。
\end{enumerate}
...
读出变体 \texttt{readout=e|b|none} 控制随机变量所在空间与是否保留瓶颈；\texttt{none} 另将块算子对齐原版 t5-small（仍保留双向 decoder、GPT-2 词表与定长 span）。
长度课程（10B、四阶段数据配比、WSD 优化器）见第~\ref{sec:curriculum} 节。
当前 Stage-1 训练矩阵（二者均 ovan-server 4\texttimes4090）见第~\ref{sec:runs} 节。
