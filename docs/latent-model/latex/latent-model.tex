...
\begin{description}
  \item[\texttt{SelfAttention}]（\texttt{encdec}，\texttt{readout=e|b}）$Q,K,V$ 投影至 $H \cdot d_{\mathrm{kv}}$；RoPE 施加于 $Q,K$。
...
\texttt{latent\_vae} 另有开关 \texttt{kl\_entropy}（不进必填键；缺键为关，以保持旧 YAML 指纹）。默认 YAML 为开：后验项改为
\begin{equation}
  \mathbb{E}[\log q] = -\tfrac{1}{2}\,\mathbb{E}\bigl[1 + \log 2\pi + \log\sigma^2\bigr]
\end{equation}
（对角高斯熵的相反数，不含 $\mu$）。最小化该项抬高 $\sigma$。开时 artifact tag 后缀 \texttt{-sigma}。\texttt{latent\_t5} 无此开关。
...
\texttt{latent\_vae} 在 \texttt{kl\_entropy} 为假（缺键）时与上式相同；为真（默认 YAML）时
\begin{equation}
  \mathcal{L} = \mathcal{L}_{\mathrm{recon}} + \beta\,\mathbb{E}[\log q] + \lambda\, \mathcal{L}_{\mathrm{aux}}.
\end{equation}
