...
\texttt{kl\_entropy} & \multicolumn{2}{c}{缺键/\texttt{false} 同旧哈希；默认 YAML 开：$\mathrm{KL}+\mathbb{E}[\log q]$} \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{latent\_t5} 与 \texttt{latent\_vae} 另有开关 \texttt{kl\_entropy}（不进必填键；缺键或 \texttt{false} 不进指纹，与旧 YAML 同哈希）。关时后验项即为上式 KL。开（默认 YAML）时两项并存、同一 $\beta$：
\begin{equation}
...
\texttt{kl\_entropy} 为假（缺键 / \texttt{false}，不进指纹）时 $\mathcal{R}=\mathrm{KL}(q\| \mathcal{N}(0,I))$；为真（默认 YAML）时
\begin{equation}
