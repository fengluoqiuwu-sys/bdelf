...
\midrule
\texttt{frozen} & 默认。全程冻结。完整 latent 参数仍写入 s2 checkpoint。 \\
\texttt{full} & 自 step 0 起更新 latent。 \\
\texttt{mid} & 已消费 token 未到 \texttt{latent\_thaw\_tokens} 时与冻结相同；到达该点后解冻一次，此后与 \texttt{full} 相同。解冻后为新可训参数分配新的优化器状态。 \\
\bottomrule
\end{tabularx}
\end{center}
...
\]
\(G\) 输出干净点估计 \(\hat x_0\in\mathbb{R}^{X}\)，再转换为速度并计算 v-MSE。分母取 \(\max(1-t,\varepsilon_t)\)，其中 \(\varepsilon_t=\) \texttt{vel\_eps}，以避免 \(t\to 1\) 时除零。干净端默认取均值 \(\mu\)（对照后验样本 \(z\)）；推理写入 KV 的对象必须与该干净端处于同一空间 \(X\)。
...
\]
后验项 \(\mathcal{R}\) 由 \texttt{kl\_entropy} 切换：缺键或假（默认 YAML）为 \(\mathrm{KL}(q_\phi\|N(0,I))\)；真为 \(\mathrm{KL}(q_\phi\|N(0,I))+\mathbb{E}[\log q_\phi]\)（对角高斯、不含 \(\mu\)，抬 \(\sigma\)）。开时 artifact \texttt{tag} 后缀 \texttt{-sigma}。\(\mathrm{CE}_{\mathrm{rec}}\) 与 BERT-mask 都经 VAE-dec。\(q_{\phi_{\mathrm{ref}}}\) 是 s2 开始时加载器给出的 encoder 冻结副本。\texttt{frozen} 时 \(\mathcal{L}_{\mathrm{s1}}=0\)。
...
tokens
  ...
  |  optional per-dim μ whiten on X (m,s after artifact export)
  |  G stem: Linear X -> D
  v
pack_2l [sg(h_left)|h_t]
  ...
  v
Exit (locked):  unwhiten x0_hat -> VAE-dec  (inference readout only)
\end{verbatim}
...
\texttt{tag} & & 交给加载器的 tag。\texttt{kl\_entropy} 开时后缀 \texttt{-sigma}。 & \texttt{100m-b32-d1} \\
\texttt{kl\_entropy} & & s1 后验项：真则 \(\beta(\mathrm{KL}+\mathbb{E}[\log q])\)，假则仅 prior-KL。不进必填键；缺键或 \texttt{false} 不进指纹。 & \texttt{false} \\
...
\texttt{latent\_tune} & & 加载之后是否训练 latent 本体。三档对应三个训练配置哈希。与日程档 \texttt{mid} 不是同一键。 & \texttt{frozen} \\
...
\texttt{latent\_tune} & & \texttt{frozen} & \texttt{mid}、\texttt{full}（主网格；日程档 \texttt{full} 下每档各走 \(B_{\mathrm{main}}+B_{\mathrm{ext}}\)） \\
\texttt{x0\_source} & & \texttt{mu} & \texttt{z} \\
\bottomrule
\end{tabularx}
\end{center}
}
...
CFG 含两根相互独立的轴。\(w_{\mathrm{sc}}\) 进入右段未知槽 AdaLN（未知一律 \(m=\mathrm{denoise}\)），推理为单次前向。\(w_{\mathrm{ctx}}\) 在训练时以概率丢弃前缀；推理默认 \(w_{\mathrm{ctx}}=1\)，仅带前缀一次前向（\(w_{\mathrm{ctx}}\neq 1\) 时另需无前缀支路，共两次前向）。推理时 VAE-dec 只读取末次流的 \(\hat x_0\)。无「纯 decode 步」：每跳都可跑 teacher。
...
推理默认 \(w_{\mathrm{ctx}}=1\)，仅运行带前缀的一次前向。\(w_{\mathrm{ctx}}\neq 1\) 时做上下文外推：无条件支路为\textbf{无前缀、仅运行当前块/窗}。然后
\[
...
\texttt{w\_ctx} & \(w_{\mathrm{ctx}}\) & 推理上下文外推；\(1\)=不外推。YAML 键名即此；别名 \texttt{w\_cfg} / \texttt{ctx\_cfg\_scale} 同上。 & \(1.0\) \\
\bottomrule
\end{tabularx}
\end{center}
}
...
\texttt{whiten} & & 映射前逐维仿射标准化（\(m,s\) 由 artifact 导出后离线写入）。 & \texttt{true} \\
...
\texttt{ctx\_source} & & 2L 左段用 \(z\) 还是 \(\mu\)。锁死 \(\mu\)。 & \texttt{mu} \\
\texttt{x0\_source} & & 流的干净端 / v-MSE 靶。 & \texttt{mu} \\
