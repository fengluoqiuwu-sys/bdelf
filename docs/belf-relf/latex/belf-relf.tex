...
\begin{enumerate}
  \item BELF：将同一流用于块因果 2L/KV；块间提交末流 \(\hat x_0\)（\texttt{commit\_x0hat=true}）；已知余数钉干净条件；\(T\) 次流均算 v-MSE；跑满目标长后全文一次 VAE-dec（不多一次 \(G\)，块内不读词）。
...
喂给 VAE-dec 的是全文已提交码。推理 \texttt{block\_generate} / \texttt{rolling\_generate} 在循环内只提交 \(\hat x_0\)（已知余数仍钉 encoder 码），跑满目标长后对整段一次 VAE-dec。因此：BELF / RELF 推理左段都是提交的 \(\hat x_0\)（外部前缀完整块仍走 encoder）；读词不发生在块/窗循环内。NFE 高于按 EOS 早停。训练窗的句首/句尾切仍按数据里的 BOS/EOS；推理滚动不依赖尚未读出的 EOS。
...
\paragraph{self-left。}
\label{sec:self-left}
推理左段默认是提交的 \(\hat x_0\)（\texttt{commit\_x0hat=true}；热路径已删除再 encode 词）。训练默认仍是 encoder 干净码。累计 token 未到 \texttt{self\_left\_thaw\_tokens}（默认 \(5\mathrm{B}\)）时 \(p_{\mathrm{left}}=0\)；到点后以概率 \texttt{self\_left\_prob}（默认 \(0.25\)，eval / 生成视为 0）按样本把左段换成 stop-grad 的末流 \(\hat x_0\)：先抽 Bernoulli，仅命中行在 CFG 之前用 GT 左 + 右段钉 \(t=L_{T-1}\)、\(m=\mathrm{denoise}\) 做一次 \texttt{no\_grad} \(G\)（与推理末跳同条件，取 x-pred，不 Euler），写回 \(h_{\mathrm{left}}\)，再跑 teacher / 学生。未命中行不跑该次 \(G\)。\(0\) 表示一开始就开（对照用，不列消融）。已知余数与 PAD 仍钉 encoder 干净码。这不是当前训练 hop 的 \(\hat x_0\)。BELF 与 RELF 共用。
...
\]
因而 \(L_0=0\)，\(L_T=Q(1)=1-\varepsilon\)。第 \(i\) 跳（\(i=0,\ldots,T-1\)）在 \(t=L_i\) 上跑 \(G\)，\(\Delta t=L_{i+1}-L_i\)，末次 Euler 到 \(1-\varepsilon\)。推理循环只提交 \(\hat x_0\)，全文一次 VAE-dec，不再于 \(t=1-\varepsilon\) 多跑一次 \(G\)。默认 \(T=16\)、\(P_m=-1.5\)、\(P_s=0.8\)（\(\mathrm{logit}(t)\sim\mathcal{N}(-1.5,0.8^{2})\)，质量偏向更噪端，压低末档 \(1/(1-t)^{2}\)）。训练与推理共用该确定梯子：推理不再从 logit-normal 随机采样，也不使用 \(\mathrm{linspace}(0,1)\)。
...
已知槽钉 \(t=1\)，不加 \(m\)、不加噪。主跑 \(W=T\)。切块对齐 \(0,W,2W,\ldots\)。已提交的完整块进入 KV，流仅作用于当前块的未知槽。推理 \texttt{block\_generate} 共 \(T\) 次 \(G\)：跳 \(i=0,\ldots,T-1\) 未知槽 \(t=L_i\)，Euler \(\Delta t=L_{i+1}-L_i\)（末流从 \(L_{T-1}\) 走到 \(1-\varepsilon\)）；然后提交 \(\hat x_0\)。跑满目标长后全文一次 VAE-dec，不多一次 \(G\)。已知槽每跳钉 \(t=1\) 并覆写为 encoder 干净码。混合块如何自生成、如何训，见第~\ref{sec:belf-clean}~节。
...
\begin{enumerate}
  \item \textbf{已齐的完整块}写入 KV。同一次生成循环里刚写完的块提交 \(G\) 的 \(\hat x_0\)（\texttt{commit\_x0hat=true}；热路径已删除再 encode 词）。若这次调用的前缀是外部文本（提示），完整块走 encoder、\texttt{sample=false}。这是左段干净条件，不是当前块内的余数。
  \item \textbf{当前块已知余数}只在 \(r>0\) 时出现：槽 \([0,r)\) 钉 \(t=1\)，码是前缀对应位置的 encoder 干净码，不进入损失。这 \(r\) 个 token 可以是提示。\(r=0\) 则整块未知。
  \item 未知槽从噪声走 \(T\) 次流。每跳之后把已知槽覆写回 encoder 干净码并钉 \(t=1\)，避免 Euler / SDE 冲掉已知位。末次 Euler 后提交该跳 \(\hat x_0\)，不读词。
  \item 把本块 \(\hat x_0\) 提交进 KV。此后下一块整块未知：自生成的完整块只出现在左段 KV，不再以块内余数出现。
  \item 跑满目标长后全文一次 VAE-dec，再 \texttt{pad\_after\_first\_eos}。块内不读词、不按 EOS 早停；NFE 高于早停。
\end{enumerate}

\paragraph{训练。}
以概率 \texttt{clean\_block\_prob} 把当前块抽成混合块：有效长 \(W'\)（末块可短），切点 \(r\sim\mathrm{Unif}\{1,\ldots,W'-1\}\)。槽 \([0,r)\) 为已知（\(t=1\)、不进损失），\([r,W')\) 为未知（走该跳 \(t=L_i\)）。其余前向整块均为未知。\(W=1\) 或 \(W'\le 1\) 时不抽混合块。已知槽始终是 encoder 码，不用 \(\hat x_0\) 冒充余数。右段 unknown Q 不可见同块 PAD K。可训档硬拒入口块长 \(\neq 1\)，不必二次 encode。仅冻结档且入口块长 \(=W\) 时，余数再 encode 一份条件句（当前块 \([r,W)\) 写成 PAD），已知覆写与 PAD 干净码用这份；插值靶仍来自整句 encode。左段 KV 训练时默认是 encoder 干净码（教师强制），累计 token 到 \texttt{self\_left\_thaw\_tokens} 后以概率 \texttt{self\_left\_prob} 换成末流 \(\mathrm{sg}(\hat x_0)\)（见第~\ref{sec:self-left}~节）；推理左段默认是提交的 \(\hat x_0\)。
...
\caption{BELF：已完成的前块不算损失。\(T\) 次流的未知槽都算速度 MSE。橙为末流档（该步 Euler 后提交 \(\hat x_0\)），仍是 v-MSE。}
\label{fig:belf-pack}
\end{figure}
...
\texttt{block\_generate}。前缀如何切成「完整块进 KV + 当前块左侧已知余数」、每跳如何覆写已知槽、如何提交 \(\hat x_0\)，见第~\ref{sec:belf-clean}~节。未知槽共 \(T\) 次 \(G\)：跳 \(i=0,\ldots,T-1\) 未知槽 \(t=L_i\)，Euler \(\Delta t=L_{i+1}-L_i\)（末流从 \(L_{T-1}\) 到 \(1-\varepsilon\)）；然后提交 \(\hat x_0\)。只要总长未到 \texttt{max\_seq\_len}，后续块整块未知（末块可短）。不按 EOS 早停；跑满后全文一次 VAE-dec。\texttt{sc\_cfg} 为真时：未知槽 sc 使用上一步 \(\hat x_0\)，AdaLN 以推理 \(w_{\mathrm{sc}}\) 为条件；新块起始与已知槽为 \(0\)。采样器见第~\ref{sec:infer}~节。
...
生成分为三段。Freeroll 为滚动阶段：整窗铺 \(F\)，梯子有 \(T\) 档，每档 \(S\) 个槽，最左档 \(t=L_{T-1}\)、右侧纯噪。每步一次 \(G\)：未知档（含最左）各升\textbf{一档}（最左 Euler 到 \(1-\varepsilon\)），把最左该步 \(\hat x_0\) \textbf{提交}并 pop 这 \(S\) 槽（不读词），右端补入 \(S\) 个新噪声，回到 freeroll。\(S\) 只表示档宽与 pop 个数。Preroll 把同一把 \(F\) 铺在虚拟满窗上：\textbf{BOS 之前整段截掉}。训练窗另可按数据 EOS 做句尾切。推理滚动跑到目标长，不依赖尚未读出的 EOS。总长上限为 \texttt{max\_seq\_len}。
...
\paragraph{自生成。}
\texttt{rolling\_generate}。新码一律从 \(L_0\) 爬到 \(L_{T-1}\) 再 Euler 到 \(1-\varepsilon\) 才提交 / pop；余数只钉干净，不让新槽跳过梯子。循环内不读词。
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \(h=T-1\)：\(k_0=0\)，即混合满窗 \(F\)。最左档 \(t=L_{T-1}\) 亦 Euler 到 \(1-\varepsilon\)；未知最左格训练算 v-MSE，推理提交该步 \(\hat x_0\) 并 pop 这 \(S\) 槽（已知 \(r\) 来自前缀 encoder，新写 \(S-r\) 为 \(\hat x_0\)）。随后 freeroll，窗内不再带余数。
  \end{itemize}
  \item BOS 若落在已知余数里：钉干净，不当未知，也不踢出窗。
  \item 同一次滚动里刚 pop 的槽提交 \(\hat x_0\)；外部前缀完整 pop 仍先 encode 再提交。跑满目标长后全文一次 VAE-dec。
\end{itemize}
...
\end{itemize}
不为凑档截掉 BOS / EOS 或把 PAD 当截断对象。已知槽与左段训练时默认都是 encoder 码（教师强制）；累计 token 到 \texttt{self\_left\_thaw\_tokens} 后以概率 \texttt{self\_left\_prob} 把左段换成末流 \(\mathrm{sg}(\hat x_0)\)。推理左段默认是提交的 \(\hat x_0\)。训练句尾切仍按数据 EOS；推理滚动不依赖未读出的 EOS。
...
续写全过程见图~\ref{fig:relf-clean-cont}：这 \(T\) 帧就是 \(L>0,r>0\) 的推理。余数 \(K\) 从虚拟窗右端进入、随 hop 左移 \(S\)；其右第一个新槽沿 \(F\) 从 \(L_0\) 升到 \(L_{T-1}\) 再 Euler 到 \(1-\varepsilon\)，全程打 v-MSE，末档才提交 \(\hat x_0\) / pop。不画随后的 freeroll。

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{relf_clean_cont.pdf}
\caption{RELF clean 续写（\(W=8\)，\(S=2\)，\(r=1\)）。虚拟满窗左切：绿格 K 第一帧在 \(k{=}6\)（靠右），之后每次左移 \(S\)。红框为即将提交的 latent：全程打 v-MSE，末流档 \(L_{T-1}\) Euler 后才提交。末帧 K 在 \(k{=}0\)，即混合窗 \(F\)，对应图~\ref{fig:relf-clean}。不画第一次 pop 之后的 freeroll。读词只在全文结束。}
...
\caption{RELF 画的是虚拟满窗 \(F\)（\(W=8\)，每档 2 槽）。黄净、黑噪。橙=最左流档 \(t=L_{T-1}\)（该步 Euler 到 \(1-\varepsilon\) 后提交 \(\hat x_0\)）；蓝=更噪流档。灰格是 BOS 之前或（训练）EOS 之后，不在窗、不赋 \(t=0\)。前三列是只切一侧或都不切；右列两端都切。损失按切完后的 \(F_k\) 按格计算，不是三选一。}
...
\caption{RELF 推理提交（\(W=8\)，每档 \(S=2\)）。上行句首切：窗起点 \(u\) 每次右移 \(S\)，BOS 在虚拟满窗上左移；前三帧最左流档仍在 BOS 之前（已截掉），无提交；末帧 BOS 落到 \(k=0\)，该档 Euler 后提交 \(\hat x_0\) 并 pop。下行训练句尾切：数据 EOS 之后丢掉、不再右补噪声。推理滚动不依赖未读出的 EOS，跑满后再全文 VAE-dec。灰格不在窗，不赋 \(t=0\)。}
\label{fig:relf-emit}
\end{figure}
...
默认 \texttt{commit\_x0hat=true}：将 \(G\) 的 \(\hat x_0\)（流空间 \(X\)）写入 \(G\) 的 KV；跑满目标长后全文一次 VAE-dec。热路径已删除再 encode 词，推理消融不再列「关=再 encode」对照。外部前缀完整块仍走 encoder、\texttt{sample=false}。训练时左段默认是 encoder 的干净码，累计 token 到 \texttt{self\_left\_thaw\_tokens} 后以概率 \texttt{self\_left\_prob} 换成末流 \(\mathrm{sg}(\hat x_0)\)。VAE-dec 无状态，不维护 KV。提交时左段 \(t=1\)，AdaLN 不以 \(w_{\mathrm{sc}}\) 为条件，sc 通道为 0。不按尚未读出的 EOS 早停，NFE 高于早停。
...
\midrule
\multicolumn{4}{@{}l}{\textit{推理}} \\
\texttt{commit\_x0hat} & & 提交 \(\hat x_0\)；热路径不支持再 encode 词。 & \texttt{true} \\
...
对照按下列网格执行。同一 checkpoint。\(w_{\mathrm{sc}}\) / \(w_{\mathrm{ctx}}\) 扫描见第~\ref{sec:cfg}~节。评测长度见附录~\ref{sec:len}。\texttt{sde\_gamma} 取值对齐 ELF 已报告的工作点：论文默认 \(1.0\)；32 步设定 \(1.5\)（本文档默认）；8/16 步设定 \(2.0\)。\(\gamma=0\) 时 SDE 退化为 ODE，与 \texttt{sampling\_method=ode} 等价，故消融不另列 \(\gamma=0\)。\texttt{commit\_x0hat} 锁死 true，不进本表。
...
主跑 \texttt{cond\_mode=clean}。自生成与训练抽样见第~\ref{sec:belf-clean}~节、第~\ref{sec:relf-clean}~节，此处不复述。RELF：未做句首切时抽余数爬梯的一帧（\(h\sim\mathrm{Unif}\{0,\ldots,T-1\}\)，新槽从 \(L_0\) 爬到 \(L_{T-1}\) 再 Euler 到 \(1-\varepsilon\) 才提交 / pop）；句首切不抽 \(r\)。禁止钉满窗 \(F\) 左侧一次提交。任意位置填空不在本文档范围。已确定。
...
\begin{itemize}
  ...
  \item BELF：跳 \(i=0,\ldots,T-1\) 未知槽一律 v-MSE（可 CFG）；推理循环只提交 \(\hat x_0\)，跑满后再全文一次 VAE-dec。
  \item RELF：仍在窗的未知真槽一律 v-MSE（可 CFG）。最左档 \(t=L_{T-1}\)，该步 Euler 到 \(1-\varepsilon\) 后提交 / pop。训练两种切法可叠加，损失按格、不按三行表；推理滚动不依赖未读出的 EOS。
...
\textbf{问题。}
链式法则写的是 \(p_G(z^{(b)}\mid z^{(<b)})\)。推理左段默认是提交的 \(\hat x_0\)；训练若始终喂 encoder 干净码（教师强制），则右段去噪习惯「完美前缀」，一到推理左边带自生成误差就崩。这是左条件上的暴露偏差。用当前训练 hop（随机 \(t\)）的 \(\hat x_0\) 冒充左段也不行：离提交码太远。块内已知余数与 PAD 另有钉干净规则，不应拿 \(\hat x_0\) 冒充。

\textbf{解决方案。}
累计 token 未到 \texttt{self\_left\_thaw\_tokens}（默认 \(5\mathrm{B}\)，进指纹、不进消融）时不跑 self-left。到点后以概率 \texttt{self\_left\_prob}（默认 \(0.25\)，eval / 生成视为 0）做 self-left scheduled sampling：先抽 Bernoulli，仅命中行在 CFG 之前用 GT 左 + 右段钉 \(t=L_{T-1}\)、\(m=\mathrm{denoise}\) 做一次 \texttt{no\_grad} \(G\)（与推理末跳同条件，取 x-pred，不 Euler），把 \(h_{\mathrm{left}}\) 换成 \textbf{stop-grad 的末流 \(\hat x_0\)}，再跑 teacher / 学生。未命中行不跑该次 \(G\)。已知余数与 PAD 仍钉 encoder 干净码。BELF 与 RELF 共用。消融 \(\{0,0.125,0.25,0.375,0.5\}\)（解冻点不当对照轴）。它不消除「插值 \(z\) vs Euler \(z\)」那条缺口，只缓解左条件分布差。机制见第~\ref{sec:self-left}~节。已确定。
