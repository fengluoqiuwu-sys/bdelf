...
\]
因而 \(L_0=0\)，\(L_T=Q(1)=1-\varepsilon\)。第 \(i\) 跳（\(i=0,\ldots,T-1\)）在 \(t=L_i\) 上跑 \(G\)，\(\Delta t=L_{i+1}-L_i\)，末次 Euler 到 \(1-\varepsilon\)。推理循环只提交 \(\hat x_0\)，全文一次 VAE-dec，不再于 \(t=1-\varepsilon\) 多跑一次 \(G\)。默认 \(T=16\)、\(P_m=0\)、\(P_s=1.2\)（不用 \(0.8\)：那会把档挤在 \(t\approx 0.5\)，首步 \(\Delta t\approx 0.23\)、末次 \(L_{15}\to 1-\varepsilon\) 又过大；\(1.5\) 又太接近均匀）。训练与推理共用该确定梯子：推理不再从 logit-normal 随机采样，也不使用 \(\mathrm{linspace}(0,1)\)。
...
\texttt{denoiser\_p\_std} & \(P_s\) & 同上，标准差。 & \(1.2\) \\
...
\paragraph{启动开关。}
\texttt{sc\_cfg} 默认 \texttt{false}（主跑关闭以减计算），进入模型指纹，因其决定网络是否包含 ScaleEmbedder 与 self-cond 通道。为假：不构建 ScaleEmbedder、无 sc 通道、AdaLN 不以 \(w_{\mathrm{sc}}\) 为条件（仍逐列加 \(m\)）、\(v_{\mathrm{tgt}}=v_z\)、不做 teacher 的 \(\Delta v\) 前向；推理忽略 \texttt{w\_sc}。消融单独短训打开（\texttt{true}）时必须具备 sc 通道，且 \texttt{self\_cond=true}。
...
\midrule
\multicolumn{4}{@{}l}{\textit{模型}} \\
\texttt{sc\_cfg} & & 是否启用整套 \(w_{\mathrm{sc}}\)（含 sc 通道）。为假则无通道、不构建 ScaleEmbedder、不以 CFG 修正 \(v_{\mathrm{tgt}}\)。进入模型指纹。主跑关；消融开。 & \texttt{false} \\
\midrule
\multicolumn{4}{@{}l}{\textit{训练}} \\
...
对照按下列网格执行。主跑 \texttt{sc\_cfg=false}；打开 \(w_{\mathrm{sc}}\) 须单独短训。推理项使用同一 checkpoint。按槽拆损失已确定，见第~\ref{sec:open}~节，不作为消融项。\texttt{ctx\_p\_drop} 不进消融。
...
\midrule
\multicolumn{4}{@{}l}{\textit{模型}} \\
\texttt{sc\_cfg} & & \texttt{false} & \texttt{true}（单独短训打开整套 \(w_{\mathrm{sc}}\) 与 sc 通道） \\
\midrule
\multicolumn{4}{@{}l}{\textit{推理}} \\
