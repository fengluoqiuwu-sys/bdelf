...
本步无读出位置时 \(\mathcal{L}_{\mathrm{ce}}=0\)。逐 token \(\|\cdot\|^2\) 对 \(X\) 取平均。左段与已知槽不进入 \(\mathcal{L}\)。
...
**余数爬梯。** 前缀 \(L\) 不对齐档界时 \(r=L\bmod S>0\)。新词从 \(L_0\) 爬到 \(1-\varepsilon\) 才 CE/pop，禁止钉满窗 \(F\) 左侧一次 CE。hop \(h=0,\ldots,T-1\) 的虚拟余数起点 \(k_0=S(T-h-1)\)；\(h<T-1\) 时 decode 档已切掉（\(\sum m_c=0\)，只 MSE、不 pop）；\(h=T-1\) 时 \(k_0=0\)，未知 decode 格纯 CE 后 pop。

**对齐前缀冷启动。** \(L>0,\,r=0\) 且尚无 \(z_{\mathrm{carry}}\)（对齐前缀第一次滚动）时，不得把整窗标满 \(F\) 当 freeroll：训练从未见过「纯噪却钉左档 \(t=1-\varepsilon\)」。须走与余数相同的 \(T\) 帧爬梯（known 为空，\(n_{\mathrm{write}}=S\)，末 hop 再 pop 并写出 carry）。**\(r=0\) 仅在已有 carry 时跳过爬梯**（preroll / 上次 pop 之后）。无条件 \(L=0\) preroll 不变。
...
5. **出口。** 无层数。`linear`（ELF）从 \(G\) 隐状态 \(D\to V\)；`decoder`（Cola）时 \(\hat x_0\) 已在 \(X\)，直接走加载的 VAE-dec。BELF 训练只喂右段 \(\hat x_0\)，推理拼接已提交干净码。RELF decoder 前缀是 \(G\) 左段 \(\hat x_0\)；丢掉格不 scatter 进因果前缀。CE 回 \(G\)。
