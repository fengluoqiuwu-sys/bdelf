...
description: >-
  ...
  branches, record ideas under temp/<idea>/, run fast local smoke-tests, push to
...
- **开新分支前重读（关键）**：每产生一个新分支（fork 根分支、架构调整 `-<variant>`、换向）之前，
  **必须重新通读一遍相关思路与实现**，再做决定：
  - 读 `temp/<idea>/` 下的思路记录（`README.md`、各实验/调整 `.md`、放弃原因）——现在在哪一步、
    父节点是谁、之前试过什么、为何放弃/调整；
...
- **分支粒度（hybrid），分两类新分支**：
  - **架构变体**（如改 embedding 格式、改注意力结构等，属同一思路的演进）→ 从**当前分支** fork
    新分支 `<idea>-<variant>`（继承现有实现，在此基础上改），记录在**原 `temp/<idea>/`** 下
    （新增一个 `<variant>.md`）。
  - **全新想法**（与现有实现无关、独立方向）→ 从 `master` fork 新根分支，并**新建独立**
    `temp/<newidea>/`（放弃原因写回原 `temp/<idea>/`）。不在原思路目录里混入新想法记录。
  - 同思路内纯参数微调 → **留在当前分支**，用 `temp/<idea>/<实验>.md` + 带命名 run/config 区分，
    不为此开分支（避免分支爆炸）。
- 维护分支拓扑可追溯：`git log --graph --oneline` 看祖先链；每个分支在 `temp/<idea>/` 下记录
  父分支名、改动内容与原因，保证任何时刻能从某分支恢复现场继续。
- 每个思路/变体在 **本地 `temp/<idea>/`** 下建目录记录：
  - `README.md`：原始思路/架构/规格（复用 `temp/ar2.md` 这类已有写法：记号、序列构造、公式）。
  - 每个调整/子实验一个 `.md`：写明改动内容、原因、父节点、结果数据。
  - `SUMMARY.md`（只在完成后写）：整个思路的结论与最终建议。
- **换向/停止训练时的权重留存**（见第 10 步）：需保留一份最新权重时，**不放进 `temp/`**
  （push/pull 屏蔽且不宜放大文件），而是拷贝到 **`cache/temp/<idea>-<variant>/`** 下留存。
- 远端 `temp/` 与本地 `temp/` **互不同步**：远端 `~/source/bdelf/temp/agent/` 只做 AI job 登记
  （见 train-ops），思路记录放本地 `temp/<idea>/`。
...
11. 完成 → 写 temp/<idea>/SUMMARY.md → 结束并总结
```
...
- 思路/实验记录（`temp/<idea>/*.md`）属本地记录、不同步；`temp/` 在 `.gitignore` 中，版本化时用
  `git add -f temp/<idea>/...`。是否提交不影响远端 push。
...
- 处理旧 checkpoint：
  - **保留一份最佳/最近的基准 checkpoint** 在 `cache/temp/<idea>-<variant>/`（作为对照与回退点）；
  - 其余旧 run 的 checkpoint 删除（仅限 AI 登记任务范围 —— 这是对"远端只读、仅 temp 可写"
    rule 的显式例外；不要动他人 run）。
- 在 `temp/<idea>/` 记本次调整原因与基准位置。
- 重新 `push` 后回 4/5。
...
- **架构变体（同一思路的演进）**：
  ...
  - 从**当前分支** fork（`git switch -c <idea>-<variant>`），继承已实现代码，在此之上修改，
    绝不回 master 从零恢复。
  - 记录在**原 `temp/<idea>/`**（新增 `<variant>.md`，注明父分支、改动、放弃/调整原因）。
  - 处理旧 run checkpoint：留存一份最新权重到 `cache/temp/<idea>-<variant>/`，其余删除。
- **全新想法（切换方向）**：
  - 从 `master` fork 新根分支，新建独立 `temp/<newidea>/`。
  - 旧想法的放弃原因写回原 `temp/<idea>/`；如需保留，最新权重存 `cache/temp/<idea>/`。
...
- 写 `temp/<idea>/SUMMARY.md`：最终架构、最佳配置、数据、优缺点、后续建议。
